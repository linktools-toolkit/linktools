...
migration = r'''#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
from ._store import RecordQuery, StateStore, StoredRecord, sequence_key
...
async def migrate_v1_agent_identity_state(
    ...
    *,
    ...
    """Normalize the known legacy V1 Agent identity shapes in place.

    A per-domain StateStore sequence is the completion marker, so normal
    startup does not rescan history.  Markers are written only after every
    recognized record has been normalized.  A crash before that point is safe:
    the migration is idempotent and current records are never rewritten.
    """
    if not state.ready:
    ...
    # Build and validate every old-digest -> current-binding relation before
    # writing anything.  This prevents a malformed execution from partially
    # migrating its Session first.
    ...
    # execution seals.  Replace only their identity field; preserve `digest`.
    for domain in (
...
async def _mark_migration_complete(repository: object, domain: RuntimeDomain) -> None:
    ...
    async def mutate(transaction: object) -> None:
...
def _migrate_recovery_data(
    ...
    if keys != _LEGACY_RECOVERY_INPUT_FIELDS:
        raise AIError(ErrorCode.STORAGE_INTEGRITY_ERROR)
    if binding is None or _is_current_binding(binding):
        if binding is None:
            raise AIError(ErrorCode.STORAGE_VERSION_UNSUPPORTED)
        raise AIError(ErrorCode.STORAGE_INTEGRITY_ERROR)
    legacy_digest, migrated = _migrate_legacy_binding(binding, compiler)
...
(root / 'linktools-ai/src/linktools/ai/runtime/state/_migration.py').write_text(migration, encoding='utf-8')

# Extend focused tests with recovery and projection migration coverage.
path = root / 'tests/ai/test_runtime_state_identity_migration.py'
text = path.read_text(encoding='utf-8')
if 'RecoveryAdmissionRecord' not in text:
    text = text.replace(
...
if 'RuntimeDomain' not in text.split('from linktools.ai.runtime.state._contracts')[0]:
    text = text.replace(
...
@pytest.mark.asyncio
async def test_migration_rewrites_legacy_recovery_shape_without_agent_id() -> None:
...
async def test_migration_preserves_historical_projection_digest() -> None:
    ...
    try:
        ...
        await state.close()
'''
if 'test_migration_rewrites_legacy_recovery_shape_without_agent_id' not in text:
    text += append
path.write_text(text, encoding='utf-8')

print('exact Runtime V1 migration closure staged')
