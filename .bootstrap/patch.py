...
REL = "linktools-ai/src/linktools/ai/runtime/_runtime_service.py"
path = Path(REL)
text = subprocess.check_output(("git", "show", f"{BASE}:{REL}"), text=True)
text = text.replace("AgentDefinitionCatalog", "AgentCatalog")


def replace_span(start: str, end: str, replacement: str, *, after: int = 0) -> None:
    global text
    left = text.index(start, after)
...
const_start = text.index("_AGENT_TASK_LEGACY_V1_FIELDS")
const_end = text.index("\n\n\nclass _LocalRuntimeCoordinatorPort", const_start)
text = text[:const_start] + '_AGENT_TASK_V1_FIELDS = frozenset(\n    {"type", "version", "binding", "user_prompt", "planning", "thinking"}\n)' + text[const_end:]
text = text.replace(
    "    async def resume(\n        self,\n        binding_digest: str,\n        session_id: str,\n        request: ResumeSessionRequest,\n    ) -> ExecutionHandle: ...",
    "    async def resume(\n        self,\n        agent_digest: str,\n        binding_digest: str,\n        session_id: str,\n        request: ResumeSessionRequest,\n    ) -> ExecutionHandle: ...",
    1,
)
runtime = text.index("class Runtime:")
replace_span(
    "    def agent(",
    "    def _definition(",
'''    def agent(
...
# Remove duplicate import accidentally introduced in the previous executor pass.
executor = Path("linktools-ai/src/linktools/ai/agent/_executor.py")
value = executor.read_text(encoding="utf-8")
first = value.find("from ._binding import AgentBinding")
second = value.find("from ._binding import AgentBinding", first + 1)
if second >= 0:
    end = second + len("from ._binding import AgentBinding\n")
    value = value[:second] + value[end:]
executor.write_text(value, encoding="utf-8")

print("runtime restored and refactored")
