...
# Export the migration boundary from runtime.state, not top-level linktools.ai.
path = root / 'linktools-ai/src/linktools/ai/runtime/state/__init__.py'
text = path.read_text(encoding='utf-8')
anchor = 'from ._memory import MemoryStateStorageGroup, MemoryStateStore\n'
if 'from ._migration import migrate_v1_agent_identity_state\n' not in text:
    text = text.replace(anchor, 'from ._migration import migrate_v1_agent_identity_state\n' + anchor)
if '    "migrate_v1_agent_identity_state",\n' not in text:
    marker = '    "wire_type_id",\n'
    if marker in text:
        text = text.replace(marker, marker + '    "migrate_v1_agent_identity_state",\n')
    else:
        end = text.rfind(']')
        text = text[:end] + '    "migrate_v1_agent_identity_state",\n' + text[end:]
path.write_text(text, encoding='utf-8')

# Run migration after RuntimeState is materialized but before any normal service read.
...
needle = '    if not state.ready or not assets.ready:\n        raise AIError(ErrorCode.RUNTIME_DEPENDENCY_NOT_READY)\n'
replacement = needle + '    migrated = await migrate_v1_agent_identity_state(\n        state, catalog, compiler, tenant_id=tenant_id\n    )\n    if migrated:\n        _logger.info("legacy Runtime V1 state normalized: records=%s", migrated)\n'
if replacement not in text:
    text = text.replace(needle, replacement)
path.write_text(text, encoding='utf-8')

# Add focused migration tests without weakening normal codec tests.
...
async def test_migration_rejects_legacy_execution_without_exact_snapshot() -> None:
    ...
    try:
        ...
        await state.close()
'''
(root / 'tests/ai/test_runtime_state_identity_migration.py').write_text(test, encoding='utf-8')

print('runtime V1 agent identity migration staged')
