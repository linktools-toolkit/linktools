...
# Freeze fingerprints for the current pre-release V1 dataclass shapes before replacing
# the explicit execution/recovery codecs. The descriptor depends on dataclass types, not
# the encoder implementation, so these are the intended current V1 fingerprints.
...
# Re-pin every changed current V1 dataclass schema fingerprint. This project is
# pre-release and the final spec explicitly keeps the current format at V1 rather
# than carrying a legacy development V1 branch.
...
for forbidden in (
    ...
    'legacy execution without binding',
    'legacy recovery input without binding',
):
    if forbidden in text:
        raise RuntimeError(f'legacy V1 residue remains in state codec: {forbidden}')
...
# Keep the final public signature from the spec while avoiding false-positive
# cohesion classification of the intentionally heterogeneous object parameter.
...
if text.count(old) != 1:
    raise RuntimeError(f'workspace build bindings annotation mismatch: {text.count(old)}')
path.write_text(text.replace(old, new, 1), encoding='utf-8')
print('workspace public heterogeneous binding annotation retained')
