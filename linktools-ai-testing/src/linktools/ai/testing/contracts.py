...
class ArtifactBlobStoreContract(_ConformanceRecorderMixin):
    ...
    def test_open_returns_an_async_context_manager(self) -> None:
        ...
        Protocol surface) -- NOT a bare async generator or iterator.
    ...
    def test_a_non_context_manager_open_is_rejected_at_use(self) -> None:
        ...
        ``async with`` use site -- this is the negative guarantee, made
...
class ArtifactRecordStoreContract(_ConformanceRecorderMixin):
    ...
    def test_duplicate_id_same_tenant_is_idempotent_on_identical_content(self) -> None:
        """Records are create-only: a second put with the same (id, tenant) and
        byte-identical content is idempotent (returns the existing record, no
        duplicate, no raise). The record stays tenant-owned by t1."""
        store = self.record_store()

        async def _run() -> None:
            await store.put(self._record("dup-1", "t1", producer_id="first"))
            # Second put with byte-identical content -- idempotent, no raise.
            await store.put(self._record("dup-1", "t1", producer_id="first"))
    ...
    def test_duplicate_id_same_tenant_different_content_conflicts(self) -> None:
        """A second put with the same (id, tenant) but any different field is a
        create-only conflict -- the lineage of the prior write is never
        overwritten. A store that silently overwrote (or surfaced two records)
        would fail."""
        ...
        async def _run() -> None:
            ...
            # The original record is intact (no overwrite).
            fetched = await store.get(artifact_id="dup-1", tenant_id="t1")
            assert fetched is not None
            assert fetched.provenance.producer_id == "first"
...
class StorageFeaturesContract(_ConformanceRecorderMixin):
    ...
    ``features.transaction_scope = TransactionScope.DATABASE`` must yield a real
    ...
    def test_declared_transaction_scope_matches_transaction_behavior(self) -> None:
        """``features.transaction_scope`` MUST match ``transaction()`` behavior:
        ...
        async def _run() -> None:
            ...
            if features.transaction_scope is TransactionScope.DATABASE:
                ...
                async with storage.transaction() as uow:
                    assert uow is not None, (
                        "features.transaction_scope=DATABASE but transaction() "
                        "yielded None -- the declared capability is not "
                        "actually implemented"
                    )
            elif features.transaction_scope is TransactionScope.NONE:
    ...
    def test_streaming_blobs_feature_matches_artifacts(self) -> None:
        """A Storage that declares ``features.streaming_artifacts = True`` MUST
        ...
        async def _run() -> None:
            if features.streaming_artifacts:
                assert storage.artifacts is not None, (
                    "features.streaming_artifacts=True but storage.artifacts is "
    ...
    def test_coordination_feature_matches_coordinator(self) -> None:
        """``features.coordination_scope != NONE`` MUST mean a real LeaseCoordinator
        ...
        async def _run() -> None:
            ...
            if features.coordination_scope is not CoordinationScope.NONE:
                assert storage.coordination is not None, (
                    f"features.coordination_scope={features.coordination_scope.value!r} but "
    ...
    def test_false_feature_declaration_is_honest_about_absence(self) -> None:
        ...
        def _check() -> None:
            if not features.streaming_artifacts:
                ...
                assert features.streaming_artifacts is False
