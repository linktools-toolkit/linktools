...
```
[初始化阶段]  ← 每次执行任意 ct-cntr 命令时触发
  ContainerManager
    ├─ 扫描 container.py / docker-compose.yml，依赖解析
    └─ 逐个实例化 → container.on_init()


[准备阶段]  ← up / restart / down / config / exec 等命令执行前触发
  ContainerManager
    ├─ 解析已安装容器及依赖，设置 container.enable 状态
    ├─ 逆序遍历已安装容器，合并各容器配置默认值
    ├─ 遍历所有容器 → container.on_prepare()      # 配置就绪后的跨容器初始化
    └─ 正序遍历已安装容器，加载各容器资源
         ├─ container.docker_file                  # 渲染 Dockerfile 模板
         ├─ container.docker_compose               # 渲染 docker-compose.yml 模板
         └─ container.exposes                      # 加载对外暴露的服务链接
...
[ct-cntr up]
  ContainerManager
    ...
    │
    └─ [孤立容器清理] target_containers 为默认全部已安装容器时
