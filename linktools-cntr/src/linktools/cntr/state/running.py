#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Running-container state (refactor spec Phase 5).

The persisted RUNNING_CONTAINERS set previously drifted: only notify_remove
updated it, only for full-container runs, and `down` left containers marked as
running. This module owns the state and is updated by ComposeRunner/CLI/exec
*after* a successful compose up/down (refactor spec §9.6):
...
so ``ct-cntr list`` never crashes when Docker is absent (§9.2/§9.8). When output
capture is added, only ``get_actual`` needs to change.
"""
...
class RunningStateStore:
    ...
    def _get(self) -> "list[str]":
        return list(self.manager._load_setting(_RUNNING_KEY, reload=True, default=[]) or [])

    def _set(self, names: "Iterable[str]") -> None:
        self.manager._dump_setting(_RUNNING_KEY, sorted(set(names)))
    ...
    def get_actual(self, containers: "Iterable[BaseContainer]") -> "list[str]":
        ...
        (refactor spec §9.5). Raises so callers fall back to persisted state.
        """
