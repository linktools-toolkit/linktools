...
def get_commands(environ: "BaseEnviron") -> "Iterable[SubCommand]":
    ...
    @register_command(name="migrate", description="migrate legacy config into the new store (spec §4.9)")
    class MigrateCommand(SubCommand):

        def create_parser(self, type: "Callable[..., CommandParser]") -> "CommandParser":
            parser = super().create_parser(type)
            parser.add_argument("action", choices=("inspect", "run", "verify", "rollback"),
                                help="inspect | run | verify | rollback")
            parser.add_argument("--config-path", metavar="PATH",
                                help="legacy config file (.cfg) -- required for inspect/run/rollback")
            parser.add_argument("--migration-id", metavar="ID",
                                help="migration id whose backup to restore (rollback)")
            parser.add_argument("--backup", metavar="PATH",
                                help="explicit backup file to restore from (rollback)")
            parser.add_argument("--dry-run", action="store_true", help="plan without writing (run)")
            parser.add_argument("--json", action="store_true", dest="as_json",
                                help="emit machine-readable JSON")
            # --yes / --no-input are provided globally by the framework; they put
            # the CLI into non-interactive mode (is_no_input() == True).
            return parser

        def _emit(self, args, payload):
        ...
        def run(self, args: "argparse.Namespace"):
            ...
            mig = ConfigMigration(store, logger=environ.logger,
                                  config_dir=str(environ.paths.config))

            if args.action == "verify":
                ok = mig.verify()
                self._emit(args, {"ok": ok})
                return 0 if ok else 1

            if not args.config_path:
                raise CommandError("--config-path is required for %s" % args.action)
            old = args.config_path

            if args.action == "inspect":
                self._emit(args, mig.inspect(old))
                return 0

            if args.action == "run":
                # In interactive mode, confirm before mutating user config; in
                # non-interactive mode (--yes/--no-input or headless) proceed.
                if not args.dry_run and not is_no_input():
                    if not confirm("Migrate %s into the new config store?" % old):
                        environ.logger.info("aborted")
                        return 0
                backup = None if args.dry_run else mig.backup(old)
                report = mig.migrate(old, dry_run=args.dry_run)
                report["_backup"] = backup
                report["_verified"] = mig.verify(report) if not args.dry_run else None
                self._emit(args, report)
                return 0 if (args.dry_run or report["_verified"]) else 1

            if args.action == "rollback":
                backup = args.backup
                if backup is None and args.migration_id:
                    backup = str(environ.paths.config / "migrations" / args.migration_id / "old-config.backup")
                if not backup or not os.path.isfile(backup):
                    raise CommandError("backup not found: %r" % backup)
                mig.rollback(backup, old)
                environ.logger.info("restored %s from %s", old, backup)
                return 0
