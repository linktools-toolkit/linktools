...
Also puts ``linktools-ai/`` on ``sys.path`` so the storage conformance testkit
(``linktools-ai/testing/``, test-support code never shipped in the wheel) is
importable as ``testing``. This duplicates the same insert in the repo-root
...
_LINKTOOLS_AI_ROOT = Path(__file__).parent.parent.parent / "linktools-ai"
