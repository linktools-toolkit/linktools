...
from linktools.ai.artifact.models import ArtifactRecord
from linktools.ai.errors import (
    StorageBlobIntegrityError,
    StorageBlobNotFoundError,
)
from linktools.ai.storage.blob.protocols import BlobInfo
from linktools.ai.storage.coordination.protocols import LeaseToken
...
class InMemoryArtifactBlobStore:
    ...
    async def put_if_absent(
        ...
        if hashlib.sha256(actual).hexdigest() != digest:
            raise StorageBlobIntegrityError(
...
class InMemoryArtifactRecordStore:
    ...
    async def put(self, record: ArtifactRecord) -> ArtifactRecord:
        ...
        if existing is not None:
            # Create-only: identical content is idempotent, a different value is
            # a conflict (never a silent overwrite of lineage).
            if existing != record:
