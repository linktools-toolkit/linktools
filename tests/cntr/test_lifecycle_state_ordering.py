#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Persisted running state must reflect successful runtime changes immediately.

If post-start/post-stop callbacks fail, the command still fails but persisted
state must already describe what actually happened in the runtime. Root and
container exec entry points must preserve the same ordering.
"""
import pytest
...
def _neutralize_runtime(manager, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "plan",
        lambda model, services=(), force_pull=False: ImagePlan((), (), tuple(services)),
    )
    monkeypatch.setattr(
        manager.image_preparer,
        "execute",
        lambda context, model, services=(), force_pull=False: ImagePlan(
            (), (), tuple(services)
        ),
    )
...
def test_down_marks_stopped_before_after_stop_hook_failure(fresh_manager, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="after-stop boom"):
        fresh_manager.compose_operations.down(names=["portainer"])
...
# -- partial lifecycle also reconciles removed containers -------------------
