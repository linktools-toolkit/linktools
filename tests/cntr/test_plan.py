#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ExecutionPlanner contract.

A plan describes only work that is knowable without executing Docker or
lifecycle side effects. Runtime image pull/build classification is deliberately
excluded because it depends on the resolved Compose model and local image state.
"""
import json
import os
import subprocess
...
def _no_real_processes(monkeypatch, fresh_manager):
    """Plan may run only its read-only preflight stub, never apply commands."""
...
def test_plan_never_writes_state_or_artifact_index(fresh_manager, monkeypatch):
    calls = []
...
def test_plan_resolved_containers_include_dependencies_of_partial_target(fresh_manager):
...
def test_plan_restart_includes_only_stop_and_up(fresh_manager):
...
def test_plan_never_invents_runtime_image_phases(fresh_manager):
    normal = fresh_manager.planner.plan("up", names=["portainer"], pull=False)
    forced = fresh_manager.planner.plan("up", names=["portainer"], pull=True)
    assert [c.phase for c in normal.commands] == ["up"]
    assert [c.phase for c in forced.commands] == ["up"]
    assert normal.commands[0].args == forced.commands[0].args
    assert ("--pull", "never") == tuple(
        forced.commands[0].args[
            forced.commands[0].args.index("--pull"):forced.commands[0].args.index("--pull") + 2
        ]
    )
...
def test_plan_commands_reuse_real_up_arg_builder(fresh_manager, monkeypatch):
    recorded_options = []
    ...
    assert "up" in next(c for c in plan.commands if c.phase == "up").args
...
def test_plan_preflight_passes_by_default(fresh_manager):
    assert fresh_manager.planner.plan("up", names=["portainer"]).preflight == "passed"
...
def test_plan_down_skips_preflight(fresh_manager):
    assert fresh_manager.planner.plan("down", names=["portainer"]).preflight == "skipped"


def test_dry_run_and_helper_share_one_model(fresh_manager, monkeypatch):
    recorded = []
...
def test_plan_up_command_file_args_match_display_order(fresh_manager):
    plan = fresh_manager.planner.plan("up")
    up_command = next(c for c in plan.commands if c.phase == "up")
    file_positions = [i for i, arg in enumerate(up_command.args) if arg == "--file"]
    files_in_command = [up_command.args[i + 1] for i in file_positions]
    assert tuple(files_in_command) == plan.compose_files


def test_plan_never_contains_proxy_secret_or_build_args(fresh_manager, monkeypatch):
    ...
    payload = json.dumps(
        {
            "commands": [list(command.display_args) for command in plan.commands],
        }
    )
    assert "--build-arg" not in payload
