...
def test_sqlite_storage_builds_engine_sessionmaker_and_database_scope(tmp_path) -> None:
    ...
    async def _run() -> None:
        storage = await SqliteStorage.create(database=tmp_path / "ref.db")
        try:
...
def test_sqlite_storage_transaction_yields_session_bound_uow(tmp_path) -> None:
    ...
    async def _run() -> None:
        storage = await SqliteStorage.create(database=tmp_path / "tx.db")
        try:
...
def test_sqlite_blob_root_is_private_per_database(tmp_path) -> None:
    ...
    async def _run() -> None:
        storage_a = await SqliteStorage.create(database=tmp_path / "a.db")
        storage_b = await SqliteStorage.create(database=tmp_path / "b.db")
        try:
            ...
            await storage_a.dispose()
            await storage_b.dispose()
...
def test_sqlite_sweep_over_one_db_does_not_touch_another(tmp_path) -> None:
    ...
    async def _run() -> None:
        ...
        try:
            ...
            a_digests = [d async for d, _ in storage_a.artifacts._blob.iter_digests_with_mtime()]
...
def test_sqlite_custom_artifact_root_is_honored(tmp_path) -> None:
    ...
    async def _run() -> None:
        storage = await SqliteStorage.create(
            database=tmp_path / "c.db", artifact_root=custom
        )
        try:
...
def test_sqlite_memory_without_artifact_root_fails(tmp_path) -> None:
    ...
    # from, so the caller MUST name one explicitly. The ValueError is raised
    # synchronously inside ``create()`` (via ``_configure``) before any await.
    from linktools.ai.runtime.persistence.sqlite import SqliteStorage

    async def _run() -> None:
        with pytest.raises(ValueError):
            await SqliteStorage.create(database=":memory:")
...
def test_sqlite_uri_without_artifact_root_fails(tmp_path) -> None:
    ...
    async def _run() -> None:
        with pytest.raises(ValueError):
            await SqliteStorage.create(database=f"file:{tmp_path}/u.db")
...
def test_sqlite_dispose_does_not_delete_artifact_root(tmp_path) -> None:
    ...
    async def _run() -> None:
        storage = await SqliteStorage.create(database=tmp_path / "d.db")
