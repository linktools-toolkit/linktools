...
async def test_put_without_metadata_does_not_share_mutable_default_across_calls():
    service = ArtifactService(resources=ResourceStore(primary=MemoryResourceBackend()))
    first = await service.put(tenant_id="acme", run_id="run-1", artifact_name="a.json", content=b"1")
    second = await service.put(tenant_id="acme", run_id="run-2", artifact_name="b.json", content=b"2")
    ...
    assert first.info.metadata is not second.info.metadata or dict(first.info.metadata) == dict(second.info.metadata)
