#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""TOCTOU + lost-update regression tests for ResourceStore on SqlAlchemyResourceBackend.
...
async def test_concurrent_puts_produce_distinct_revisions(tmp_path):
    ...
    n = 8
    paths = [ResourcePath(f"/r/{i}.txt") for i in range(n)]
    ...
    async def one_put(p: ResourcePath) -> None:
...
async def test_revision_increments_monotonically_under_sequential_puts(tmp_path):
    ...
    await store.put(ResourcePath("/a.txt"), b"a")
...
async def test_revision_bump_uses_atomic_update_returning(tmp_path):
    ...
    await store.put(ResourcePath("/seed.txt"), b"x")
    captured.clear()
...
async def test_resource_update_uses_conditional_where_on_version(tmp_path):
    ...
    await store.put(ResourcePath("/u.txt"), b"v1")
    captured.clear()
