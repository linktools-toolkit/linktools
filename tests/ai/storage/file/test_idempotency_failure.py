#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""B-01 regression: FileIdempotencyStore.fail() must take the fenced claim token
(not the old (scope, key, error) signature). A shadowed old signature would
make ToolExecutor's fail(claim, error) hit the wrong method (TypeError) and
...
def test_file_idempotency_fail_signature_is_single_claim_token(tmp_path):
    ...
    sig = inspect.signature(FileIdempotencyStore.fail)
