...
def test_converged_roots_and_owners_exist() -> None:
    ...
    assert not (root / "app" / "_facade.py").exists()
    assert not (root / "app" / "_assembly.py").exists()
    assert not (root / "app" / "_workbench.py").exists()
    assert not (root / "app" / "_sql.py").exists()
    assert (root / "adapter" / "_sql_runtime.py").is_file()
    assert (root / "adapter" / "_sql_repositories.py").is_file()
    assert (root / "adapter" / "_history.py").is_file()
    assert (root / "adapter" / "_step.py").is_file()
