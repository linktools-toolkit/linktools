...
def test_converged_roots_and_owners_exist() -> None:
    ...
    assert (root / "app" / "_facade.py").is_file()
    assert (root / "app" / "_assembly.py").is_file()
    assert (root / "app" / "_workbench.py").is_file()
    assert (root / "app" / "_sql.py").is_file()
    assert (root / "adapter" / "_sql.py").is_file()
