...
from linktools.ai.agent.sandbox.workspace import (
    Workspace,
...
async def test_create_returns_workspace_ref(tmp_path):
    manager = LocalWorkspaceManager(root=tmp_path)
    ref = await manager.create(run_id="run-1", tenant_id="tenant-1")
...
async def test_two_runs_get_isolated_workspaces(tmp_path):
    manager = LocalWorkspaceManager(root=tmp_path)
    ref_a = await manager.create(run_id="run-a", tenant_id="tenant-1")
    ref_b = await manager.create(run_id="run-b", tenant_id="tenant-1")
