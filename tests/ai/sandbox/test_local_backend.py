...
from linktools.ai.agent.sandbox import local as local_module
from linktools.ai.agent.sandbox.local import LocalSandbox
from linktools.ai.agent.sandbox.protocols import Sandbox, ExecutionIsolationLevel


@pytest.fixture()
def backend(tmp_path: Path) -> LocalSandbox:
    return LocalSandbox(runtime_dir=tmp_path, base_dirs=[])
...
def test_fork_result_satisfies_protocol(backend, tmp_path):
    ...
    assert isinstance(branch, Sandbox)
