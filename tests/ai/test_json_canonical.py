...
from linktools.ai.json import canonical_json_bytes, normalize_json
...
def test_uuid_rendered_as_canonical_string():
    ...
    assert canonical_json_bytes(u) == f'"{u}"'.encode()


def test_timezone_aware_datetime_rendered_as_utc_iso():
    dt = datetime(2026, 1, 2, 3, 4, 5, tzinfo=timezone.utc)
    assert canonical_json_bytes(dt) == b'"2026-01-02T03:04:05+00:00"'
...
def test_nested_structure_normalized():
    assert canonical_json_bytes({"a": (1, 2), "b": None}) == b'{"a":[1,2],"b":null}'
