...
class _FixedPipeline:
    ...
    def __init__(self, before_tool=None, after_tool=None):
        self._before_tool = before_tool or (
            lambda e: PipelineDecision(action=PipelineAction.ALLOW)
        )
        self._after_tool = after_tool or (
            lambda e: PipelineDecision(action=PipelineAction.ALLOW)
        )
    ...
    async def before_tool(self, e):
        return self._before_tool(e)

    async def after_tool(self, e):
        return self._after_tool(e)
...
async def test_later_deny_overrides_earlier_require_approval():
    ...
    deny = _FixedPipeline(
        before_tool=lambda e: PipelineDecision(
            action=PipelineAction.DENY, reason="blocked"
        )
    )
...
async def test_sequential_modify_second_pipeline_sees_first_pipelines_edit():
    ...
    def _a_modify(e):
        return PipelineDecision(
            action=PipelineAction.MODIFY, modified_payload={**e.arguments, "a": 1}
        )

    def _b_modify(e):
        seen_by_b.update(e.arguments)
        return PipelineDecision(
            action=PipelineAction.MODIFY, modified_payload={**e.arguments, "b": 2}
        )
...
async def test_sequential_modify_applies_to_result_field_for_after_tool():
    def _a_modify(e):
        return PipelineDecision(
            action=PipelineAction.MODIFY_RESULT, modified_payload=f"{e.result}-a"
        )

    def _b_modify(e):
        return PipelineDecision(
            action=PipelineAction.MODIFY_RESULT, modified_payload=f"{e.result}-b"
        )
...
async def test_after_tool_deny_result_short_circuits_like_deny():
    composite = CompositeSecurityPipeline(
        [
            _FixedPipeline(
                after_tool=lambda e: PipelineDecision(action=PipelineAction.DENY_RESULT)
            ),
            _FixedPipeline(
                after_tool=lambda e: PipelineDecision(
                    action=PipelineAction.MODIFY, modified_payload="never"
                )
            ),
        ]
    )
...
async def test_per_step_schema_validation_denies_bad_intermediate_modify():
    ...
    def _bad_modify(e):
        ...
        return PipelineDecision(
            action=PipelineAction.MODIFY, modified_payload={"count": "bad"}
        )
    ...
    event = ToolInvocationEvent(
        tool_name="t", arguments={"count": 1}, parameter_schema=schema
    )
