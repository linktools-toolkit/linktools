...
from linktools.ai.agent.middleware.base import Middleware
from linktools.ai.agent.middleware.pipeline import MiddlewarePipeline
...
def test_run_before_model_threads_request_in_registration_order():
    ...
    assert log == [
        "M1.before_model:req",
        "M2.before_model:req->bmM1",
        "M3.before_model:req->bmM1->bmM2",
    ]
...
def test_run_after_model_threads_response_in_reverse_order():
    ...
    assert log == [
        "M3.after_model:resp",
        "M2.after_model:resp->amM3",
        "M1.after_model:resp->amM3->amM2",
    ]
...
def test_run_before_tool_threads_in_registration_order():
    ...
    out = asyncio.run(_run())
    assert log == [
        "M1.before_tool:toolreq",
        "M2.before_tool:toolreq->btM1",
        "M3.before_tool:toolreq->btM1->btM2",
    ]
...
def test_run_after_tool_threads_result_in_reverse_order_and_passes_request_through():
    ...
    async def _run():
        return await pipeline.run_after_tool(
            context=None, request="toolreq", result="res"
        )

    out = asyncio.run(_run())
    assert log == [
        "M3.after_tool:res",
        "M2.after_tool:res->atM3",
        "M1.after_tool:res->atM3->atM2",
    ]
...
def test_run_after_tool_passes_same_request_to_every_middleware():
    ...
    pipeline = MiddlewarePipeline(
        middlewares=(_ReqCapture(), _ReqCapture(), _ReqCapture())
    )

    async def _run():
        return await pipeline.run_after_tool(
            context=None, request="the-request", result="r"
        )
