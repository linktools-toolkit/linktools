#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Tests for agent/catalog.py: AgentSpecIndex resolves AgentSpec from
...
def test_get_re_reads_after_file_change(tmp_path):
    _write_agents(tmp_path)
    agents = tmp_path / "agents"
    registry = AgentSpecIndex.from_specloader(SpecLoader.from_filesystem(agents))
...
# 5. Missing agent -> SpecNotFoundError (propagated from the loader).
...
# 6b. Malformed YAML in the frontmatter -> SpecParseError.
