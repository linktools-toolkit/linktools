#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Tests for skill/catalog.py: SkillSpecIndex resolves SkillSpec from {name}.md
...
from linktools.ai.agent.skill.codec import parse_skill_spec
...
def test_get_returns_skill_spec_from_markdown(tmp_path):
    ...
    registry = SkillSpecIndex.from_specloader(SpecLoader.from_filesystem(tmp_path / "skills"))
...
# 4. Missing skill -> SpecNotFoundError.
...
# 8. Malformed YAML frontmatter -> SpecParseError.
def test_get_malformed_frontmatter_raises_parse_error(tmp_path):
    ...
    registry = SkillSpecIndex.from_specloader(SpecLoader.from_filesystem(skills))
