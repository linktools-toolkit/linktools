...
from linktools.ai.capability.entries import CapabilityEntryInfo, CapabilityEntryChange
...
class Repository:
    def __init__(self):
        self.revision = 1
        self.values = {"a": CapabilityEntryInfo("a", "agent", 1, "e1")}
        self.full_reads = 0
        self.delta_reads = 0
    ...
    async def list_changes(self, *, after_revision, through_revision):
        self.delta_reads += 1
        return (CapabilityEntryChange(2, "a", self.values["a"]),)


@pytest.mark.asyncio
async def test_metadata_stat_and_list_share_revision_snapshot():
    repo = Repository()
    snapshot = MetadataSnapshot(repo)
