...
- Durable compatibility tests cover the package durable-contract rule: current wire semantics and any real compatibility commitment only; additive ordinary fields remain readable where the current decoder is forward-compatible, while unknown versions/types and malformed known fields fail closed.
