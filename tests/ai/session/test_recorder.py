...
def test_completed_messages_includes_user_and_assistant():
    recorder = SessionRecorder()
    messages = recorder.completed_messages(
...
def test_completed_messages_omits_user_when_prompt_empty():
    """A resume continuation has no new user turn -- only the assistant
    output is recorded."""
    recorder = SessionRecorder()
    messages = recorder.completed_messages(user_prompt="", output="continued", run_id="run-2")
    assert [m.role for m in messages] == [MessageRole.ASSISTANT]
    assert messages[0].content == "continued"


def test_completed_messages_stringifies_non_str_output():
    recorder = SessionRecorder()
    messages = recorder.completed_messages(user_prompt="q", output={"a": 1}, run_id="run-3")
    assistant = [m for m in messages if m.role == MessageRole.ASSISTANT][0]
    assert assistant.content == str({"a": 1})


def test_paused_messages_is_always_empty():
    recorder = SessionRecorder()
    assert recorder.paused_messages(run_id="run-4") == ()
