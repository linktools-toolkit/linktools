#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Tests for mcp/catalog.py: MCPServerSpecIndex resolves MCPServerSpec from {name}.yaml
...
from linktools.ai.agent.mcp.index import MCPServerSpecIndex
...
def test_get_returns_stdio_mcp_spec(tmp_path):
    ...
    registry = MCPServerSpecIndex.from_specloader(SpecLoader.from_filesystem(tmp_path / "mcp"))
...
def test_get_invalid_transport_raises_invalid_spec(tmp_path):
    ...
    registry = MCPServerSpecIndex.from_specloader(SpecLoader.from_filesystem(mcp))
...
# 6. Missing mcp -> SpecNotFoundError.
...
# 9. Malformed YAML -> SpecParseError.
