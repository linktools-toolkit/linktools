...
def test_execute_raises_before_handler_when_policy_denies():
    class _Deny:
        async def evaluate(self, request, context):
            return PolicyDecision(kind=PolicyDecisionKind.DENY, rule_id="x", reason="no")
...
def test_policy_capability_on_tool_execute_error_surfaces_as_skip():
    ...
    assert tool_returns
    assert "handler crashed" in str(tool_returns[0]) or "error" in str(tool_returns[0]).lower()
