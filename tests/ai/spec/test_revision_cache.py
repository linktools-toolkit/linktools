...
from linktools.ai.storage import RevisionCache


class _FakeSource:
    """A controllable SpecSource: a revision string + a {path: text} map.
    ...
    async def read(self, path: str) -> str:
        if path not in self._items:
            raise SpecNotFoundError(f"no item at {path!r}")
...
class _IdentityCodec:
    """A SpecCodec that 'parses' by wrapping the raw text, but rejects the
    sentinel '<bad>' as a parse failure."""

    def decode(self, item_id: str, raw: str) -> str:
        if raw == "<bad>":
            raise SpecParseError(f"{item_id}: bad field at $.x")
...
def test_concurrent_get_same_key_propagates_decode_failure_to_all_waiters():
    ...
    assert all(isinstance(r, SpecParseError) for r in results), (
...
def test_not_found_is_distinguished_from_parse_failure():
    ...
    # Missing item -> SpecNotFoundError.
    with pytest.raises(SpecNotFoundError):
        _run(cache.get("missing"))
    # Present-but-unparseable -> SpecParseError (NOT not-found).
...
def test_source_and_codec_are_runtime_checkable_protocols():
    # Structural: _FakeSource satisfies SpecSource, _IdentityCodec satisfies
    # SpecCodec without inheritance.
    assert isinstance(_FakeSource({}), SpecSource)
    assert isinstance(_IdentityCodec(), SpecCodec)
