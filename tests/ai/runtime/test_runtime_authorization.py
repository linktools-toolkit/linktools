...
async def _start_claimed_run(runtime, *, run_id: str, tenant_id: str):
    store = runtime.execution.store
    await store.create_session(session_id="s", user_id=None, tenant_id=tenant_id)
    codec = AgentSpecCodec()
    value = codec.encode(_spec())
    definition = RunDefinition("agent", RunnableType.AGENT, "agent-spec.v1", value, "hash")
...
async def _pause_run(runtime, claimed, *, approval_id: str):
    store = runtime.execution.store
    now = datetime.now(timezone.utc)
    snapshot = AgentSnapshotData((), None, RunUsage(), 0)
    approval = RunApproval(approval_id, "tc-1", "some-tool", {})
...
async def test_cancel_rejects_cross_tenant_access(tmp_path):
    ...
    await _start_claimed_run(runtime, run_id="r1", tenant_id="t1")

    with pytest.raises(PrincipalAccessDeniedError):
        await runtime.cancel("r1", tenant_id="t2")
...
async def test_resume_rejects_cross_tenant_access(tmp_path):
    runtime = build_runtime(storage=LocalDirectoryStorage(tmp_path), model_resolver=make_router())
    claimed = await _start_claimed_run(runtime, run_id="r1", tenant_id="t1")
    await _pause_run(runtime, claimed, approval_id="a1")
    await runtime.decide_approval("r1", approval_id="a1", decision=ApprovalDecision.ALLOW, decided_by="reviewer", tenant_id="t1")

    with pytest.raises(PrincipalAccessDeniedError):
        await runtime.resume("r1", tenant_id="t2")

    assert (await runtime.resume("r1", tenant_id="t1")) is not None
    await runtime.aclose()
...
async def test_decide_approval_rejects_cross_tenant_access(tmp_path):
    ...
    with pytest.raises(PrincipalAccessDeniedError):
        await runtime.decide_approval("r1", approval_id="a1", decision=ApprovalDecision.DENY, decided_by="reviewer", tenant_id="t2")
    await runtime.aclose()
...
async def test_decide_approval_deny_is_terminal(tmp_path):
    ...
    record = await runtime.decide_approval("r1", approval_id="a1", decision=ApprovalDecision.DENY, decided_by="reviewer", tenant_id="t1")
    assert record.status is RunStatus.CANCELLED

    with pytest.raises(PrincipalAccessDeniedError):
        await runtime.resume("r1", tenant_id="wrong-tenant")
    await runtime.aclose()
