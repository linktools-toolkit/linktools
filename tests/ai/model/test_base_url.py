...
def test_literal_keeps_explicit_v1():
    assert (
        _resolve_base_url(_cfg("https://gateway.example.com/v1"))
...
def test_literal_keeps_custom_path():
    assert (
        _resolve_base_url(_cfg("https://gateway.example.com/custom/openai"))
        == "https://gateway.example.com/custom/openai"
    )


def test_literal_does_not_append_v1():
    assert (
        _resolve_base_url(_cfg("https://gateway.example.com"))
...
def test_append_v1_is_opt_in():
    url = _resolve_base_url(
        _cfg("https://gateway.example.com", mode="append_v1_if_missing")
    )
...
def test_append_v1_idempotent_when_v1_present():
    url = _resolve_base_url(
        _cfg("https://gateway.example.com/v1", mode="append_v1_if_missing")
    )
    assert url == "https://gateway.example.com/v1"
