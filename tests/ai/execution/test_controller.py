#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""RunController behavior. The controller is the
...
from linktools.ai.errors import ExecutionAlreadyActiveError
from linktools.ai.execution.controller import (
    ActiveExecution,
...
async def test_get_token_returns_none_for_unregistered_run():
    controller = ExecutionControllerRegistry()
...
async def test_register_exposes_token_via_get_token():
    ...
    try:
        ...
        await controller.register("run-1", ActiveExecution(task, token))
...
async def test_cancel_sets_token_and_cancels_task():
    ...
    token = CancellationToken()
    await controller.register("run-blocked", ActiveExecution(task, token))
...
async def test_cancel_is_idempotent_on_already_cancelled_task():
    ...
    token = CancellationToken()
    await controller.register("run-quick", ActiveExecution(task, token))
...
async def test_unregister_removes_registration():
    ...
    task = asyncio.create_task(_dummy())
    token = CancellationToken()
    await controller.register("run-x", ActiveExecution(task, token))
    assert controller.get_token("run-x") is token

    await controller.unregister("run-x", task=task)
...
async def test_unregister_is_idempotent():
    ...
    await controller.unregister("never-registered", task=task)
    await controller.unregister("never-registered", task=task)
    await task
...
async def test_register_overwrites_stale_entry():
    ...
    await controller.register("run-restart", ActiveExecution(old_task, old_token))
    await old_task  # completes -- simulates a stale entry

    new_task = asyncio.create_task(_new())
    new_token = CancellationToken()
    await controller.register("run-restart", ActiveExecution(new_task, new_token))
...
@pytest.mark.asyncio
async def test_register_rejects_duplicate_active_execution():
    controller = ExecutionControllerRegistry()
    ...
    try:
        await controller.register(
            "run-active",
            ActiveExecution(first, CancellationToken()),
        )
        with pytest.raises(ExecutionAlreadyActiveError):
            await controller.register(
                "run-active",
                ActiveExecution(second, CancellationToken()),
            )
    finally:
        first.cancel()
        second.cancel()
