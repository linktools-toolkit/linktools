#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""forbidden-name inventory -- the baseline that the conformance gate drives
to zero.
...
Re-introducing a forbidden name in new code fails the test, so the inventory
...
def test_forbidden_name_count_does_not_increase(pattern: str, ceiling: int) -> None:
    """Each forbidden pattern's file-count must stay at or below its ceiling.
    ...
    assert actual <= ceiling, (
        f"forbidden pattern {pattern!r} grew: {actual} files > ceiling {ceiling}. "
...
def test_legacy_inventory_summary(capsys) -> None:
    ...
    """
    lines = ["forbidden-name inventory (target: all 0):"]
