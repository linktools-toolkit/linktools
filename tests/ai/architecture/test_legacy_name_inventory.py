#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Phase 0 legacy-name inventory -- the AC-14 baseline that the refactor drives
to zero.
...
§3.2/§4.3/§4.8/§4.9). The test is a *ratchet*: each pattern's file-count must
stay at or below its recorded ceiling. Lower a ceiling as soon as a phase
migrates consumers; the target is 0 across the board by the end of Phase 8.
...
Phase 0 baseline-repair commit): ``linktools-ai/src`` + ``tests/ai``.
"""
...
# at the Phase 0 baseline; ratchet downward as phases migrate consumers.
LEGACY_CEILINGS: "dict[str, int]" = {
    "storage.resource": 0,
    "ResourceStore": 0,
    # AgentRegistry (+ SkillRegistry/MCPRegistry/ToolRegistry/SwarmRegistry) were
    # the registry-shim classes; the registry/ package is DELETED (Phase 4 op 5).
    # All docstring references to the old *Registry names were updated to the
    # current *Catalog names; locked at 0.
    "AgentRegistry": 0,
    # ProviderBundle was renamed to RuntimeDependencies (Phase 4 op 7 step 2);
    ...
    "ProviderBundle": 0,
    # linktools.ai.providers package DELETED (Phase 9 op 8 / AC-06): the
    ...
    # MappingProvider moved to _runtime.dependencies (re-exported from
    # linktools.ai.runtime). All src + tests migrated. Locked at 0.
    "linktools.ai.providers": 0,
    # linktools.ai.registry package DELETED (Phase 4 op 5): all src + tests
    ...
    # linktools.ai.task renamed to linktools.ai.jobs (Phase 7); the package +
    # all consumers + tests are migrated. Locked at 0.
    "linktools.ai.task": 0,
    # linktools.ai.security and linktools.ai.policy merged into
    # linktools.ai.governance.{security,policy} (Phase 6 op 1); the packages +
    ...
    # linktools.ai.knowledge renamed to linktools.ai.retrieval (Phase 8 op 1);
    # the package + all consumers + tests are migrated. Locked at 0.
    "linktools.ai.knowledge": 0,
}

# getattr(storage, "resources") is the implicit Artifact fallback the plan
# (§4.8 显式注入) explicitly deletes. It must be 0 once Phase 3 lands; today
# there is exactly one occurrence (task/runtime.py).
FORBIDDEN_ZERO: "dict[str, str]" = {
    r'getattr\(storage,\s*["\']resources["\']\)':
        "implicit storage.resources Artifact fallback (§4.8) must be removed",
}
...
def test_forbidden_patterns_are_absent(pattern: str, reason: str) -> None:
    """Patterns the plan outright forbids must have zero occurrences."""
