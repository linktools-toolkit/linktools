#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""TaskRuntime + TaskWorker end-to-end (plan section 28 phase-5 acceptance).

Drives the full claim → execute → commit loop over a FileStorage-backed
FileTaskStore with real asyncio timing (tiny intervals) and a SystemClock.
"""
...
def test_handler_not_found_marks_task_failed(tmp_path) -> None:
    async def run() -> None:
        storage = FileStorage(root=tmp_path)
        runtime = TaskRuntime(storage=storage, handlers={}, options=FAST)
        await runtime.create_job(
...
class _FlakyCommitStore:
    """Delegates everything to an inner TaskStore, but ``commit_success``
...
def test_transient_commit_error_is_retried_without_rerun(tmp_path) -> None:
    ...
    from linktools.ai.task.protocols import SystemClock
