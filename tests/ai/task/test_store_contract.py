#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""TaskStore Protocol contract (plan section 12).

The domain-semantic surface every TaskStore backend (file, sqlalchemy) must
...
from linktools.ai.task import ClaimedTask, TaskClaim, TaskStore
...
class _CompleteFakeStore:
    """Implements every TaskStore method -- satisfies the Protocol."""
...
def test_complete_store_satisfies_protocol() -> None:
    assert isinstance(_CompleteFakeStore(), TaskStore)


def test_stripped_store_does_not_satisfy_protocol() -> None:
    assert not isinstance(_StrippedStore(), TaskStore)
