...
from linktools.ai.storage.resource.models import Resource, ResourceInfo, ResourceKind
...
from linktools.ai.task.protocols import TaskContext, TaskRequest, TaskSuccess
...
from linktools.ai.task.store import (
    TaskClaim,
    TaskRunTimeoutError,
    UnsupportedTaskSchemaError,
    claim_matches_task,
)
from linktools.ai.task.models import resolve_effective_scopes
...
class _CountingResource:
    """Wraps a ResourceStore to count get/stat calls so the snapshot's
...
def test_snapshot_uses_single_resource_read(tmp_path) -> None:
    ...
    async def run() -> None:
        ...
        from linktools.ai.task.snapshot import snapshot_resource
        ...
        artifacts = ArtifactStore(resources)  # separate store for the sealed blob
...
def test_root_task_inherits_job_scopes_at_creation(tmp_path) -> None:
    ...
    async def run() -> None:
        store = FileTaskStore(tmp_path / "t")
        now = datetime(2026, 7, 17, 12, 0, tzinfo=timezone.utc)
        job = JobRecord(
            id="j1",
            status=JobStatus.PENDING,
