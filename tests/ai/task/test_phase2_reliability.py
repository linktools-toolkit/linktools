...
6.1 is exercised end-to-end through TaskRuntime + a FileStorage (whose
...
def test_input_artifact_reaches_handler(tmp_path) -> None:
    async def run() -> None:
        ...
        artifact_store = ArtifactStore(storage.resources)
...
def _reconcile_runtime(storage, canceler) -> TaskRuntime:
    return TaskRuntime(
...
def test_non_idempotent_orphan_run_is_finalized_not_requeued(tmp_path) -> None:
    ...
    async def run() -> None:
        from linktools.ai.task.models import SideEffectMode
