#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""FileTaskStore contract -- the reliable-task invariants over the file backend
...
These tests are the contract every TaskStore backend must satisfy; the
...
@pytest.fixture
def task_store(tmp_path: Path) -> FileTaskStore:
    clock = FakeClock(datetime(2026, 7, 16, 12, 0, tzinfo=timezone.utc))
    return FileTaskStore(tmp_path, clock=clock)
...
def test_create_claim_complete_completes_job(task_store: FileTaskStore) -> None:
...
def test_completed_task_is_not_reclaimed(task_store: FileTaskStore) -> None:
    clock = task_store._clock
...
def test_transient_failure_retries_then_succeeds(task_store: FileTaskStore) -> None:
    clock = task_store._clock
...
def test_permanent_failure_does_not_retry(task_store: FileTaskStore) -> None:
    clock = task_store._clock
...
def test_persistence_survives_reopen(tmp_path: Path) -> None:
    ...
    clock = FakeClock(datetime(2026, 7, 16, 12, 0, tzinfo=timezone.utc))
    task_store = FileTaskStore(tmp_path, clock=clock)
...
def test_recovery_tolerates_missing_attempt_file(task_store: FileTaskStore) -> None:
...
def test_recovery_fails_task_when_attempts_exhausted(task_store: FileTaskStore) -> None:
...
def test_create_task_command_creates_child(task_store: FileTaskStore) -> None:
    clock = task_store._clock
...
def test_wait_signal_command_transitions_to_waiting(task_store: FileTaskStore) -> None:
...
def test_signal_wakes_matching_waiting_task(task_store: FileTaskStore) -> None:
...
def test_missing_dependency_does_not_crash(task_store: FileTaskStore) -> None:
...
def test_max_depth_caps_child_creation(task_store: FileTaskStore) -> None:
...
def test_max_runtime_blocks_claim(task_store: FileTaskStore) -> None:
...
def test_max_attempts_blocks_claim_once_job_attempt_cap_met(
    task_store: FileTaskStore,
) -> None:
    """Budget max_attempts caps the job's aggregate attempts across all its
    tasks (not just per-task retry). Once the cap is met, no further claim."""
...
def test_bind_run_fences_on_worker_id(task_store: FileTaskStore) -> None:
...
def test_concurrent_signals_for_same_waiting_task_wake_it_once(
    ...
    from linktools.ai.task.models import TaskSignalRecord
    from linktools.ai.task.protocols import WaitSignal
...
def test_commit_success_write_fault_leaves_parent_claimed_not_half_committed(
    ...
    from linktools.ai.task.models import TaskRecord as _TR
    from linktools.ai.task.protocols import CreateTask
