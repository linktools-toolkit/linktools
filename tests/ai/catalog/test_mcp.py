#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Tests for mcp/catalog.py: MCPCatalog resolves MCPServerSpec from {name}.yaml
...
def test_get_returns_stdio_mcp_spec(tmp_path):
    ...
    registry = MCPCatalog.from_specloader(SpecLoader.from_filesystem(tmp_path / "mcp"))
...
def test_get_invalid_transport_raises_invalid_spec(tmp_path):
    ...
    registry = MCPCatalog.from_specloader(SpecLoader.from_filesystem(mcp))
...
def test_get_caches_spec_per_revision():
    ...
    registry = MCPCatalog.from_specloader(loader)
