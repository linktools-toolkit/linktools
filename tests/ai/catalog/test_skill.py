#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Tests for skill/catalog.py: SkillCatalog resolves SkillSpec from {name}.md
...
def test_get_returns_skill_spec_from_markdown(tmp_path):
    ...
    registry = SkillCatalog.from_specloader(SpecLoader.from_filesystem(tmp_path / "skills"))
...
def test_get_caches_spec_per_revision():
    ...
    registry = SkillCatalog.from_specloader(loader)
...
def test_get_malformed_frontmatter_raises_parse_error(tmp_path):
    ...
    registry = SkillCatalog.from_specloader(SpecLoader.from_filesystem(skills))
