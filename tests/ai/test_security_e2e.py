...
async def test_default_baseline_denies_dangerous_command_without_pause_on_approval(tmp_path):
    ...
    def model_fn(messages, info: AgentInfo) -> ModelResponse:
        if len(messages) <= 1:
            return ModelResponse(parts=[ToolCallPart(tool_name="bash", args={"command": "rm -rf /"})])
    ...
    spec = AgentSpec(
        id="e2e", name="e2e", model=ModelPolicy(primary="m"),
        instructions=PromptSpec(instructions="hi"),
        tools=(ToolRef(name="terminal"),),
    )
...
async def test_disabled_baseline_actually_disables_denylist_without_pause_on_approval(tmp_path):
    ...
    def model_fn(messages, info: AgentInfo) -> ModelResponse:
        if len(messages) <= 1:
            return ModelResponse(parts=[ToolCallPart(tool_name="bash", args={"command": "echo ok"})])
        return ModelResponse(parts=[TextPart(content=_bash_outcome(messages))])
...
async def test_pipeline_attached_to_runtime(tmp_path):
    ...
    def model_fn(messages, info: AgentInfo) -> ModelResponse:
        if len(messages) <= 1:
            return ModelResponse(parts=[ToolCallPart(tool_name="read_file", args={"path": "x"})])
...
class _RequireApprovalPipeline:
    ...
    async def before_tool(self, e):
        return PipelineDecision(action=PipelineAction.REQUIRE_APPROVAL, reason="need ok")
...
async def test_managed_tool_approval_pauses_and_resumes_end_to_end(tmp_path):
    ...
    registry = ModelRegistry()
    registry.register("m", model=FunctionModel(_tool_calling_model_fn(
        "read_file", {"path": "x"})))
    rt = Runtime.build(
        storage=FileStorage(root=tmp_path),
        model_router=ModelRouter(registry=registry),
...
async def test_pipeline_modify_arguments_e2e(tmp_path):
    ...
    def model_fn(messages, info: AgentInfo) -> ModelResponse:
        n = sum(1 for m in messages for p in getattr(m, "parts", [])
                if getattr(p, "part_kind", None) == "tool-return")
        if n == 0:
            return ModelResponse(parts=[ToolCallPart(tool_name="read_file", args={"path": "wrong"})])
...
async def test_exposure_policy_default_hides_write_and_terminal_tools(tmp_path):
    ...
    registry = ModelRegistry()
    registry.register("m", model=FunctionModel(
    ...
    rt = Runtime.build(
        ...
        execution=backend,
        options=CapabilityRuntimeOptions(
            tool_exposure=CapabilityToolExposurePolicy()),  # defaults: no execution tools
    )
    spec = AgentSpec(
        id="e2e", name="e2e", model=ModelPolicy(primary="m"),
        instructions=PromptSpec(instructions="hi"),
