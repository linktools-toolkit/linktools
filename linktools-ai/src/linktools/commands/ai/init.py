...
"""`lt ai init`: scaffold a project's ``.linktools/``.

Writes the default config + a default/reviewer agent + a code-review skill and
the skill-creator example (with its private grader/comparator/analyzer agents) +
a disabled GitHub MCP template. Existing files are never overwritten so re-running
``init`` on a customized project is safe."""
...
class Command(BaseCommand):
    ...
    def init_arguments(self, parser: "CommandParser") -> None:
        parser.add_argument(
            "--force",
            action="store_true",
            help="no-op kept for forward compat; existing files are never overwritten",
        )
    ...
    def run(self, args: "Namespace") -> "int | None":
        ...
        for relative, content in _SCAFFOLD:
            if write_if_missing(config_root / relative, content):
                created += 1
            else:
                skipped += 1
        self.logger.info(
