...
class EvalExecution:
    ...
    # When the executor captured a full EvalSnapshot (run record / definition /
...
@dataclass(frozen=True, slots=True)
class RunEvaluation:
    """A single evaluator's score against an execution, persisted by
    :class:`~linktools.ai.execution.store.ExecutionStore`. The same run may
    carry more than one ``RunEvaluation`` (one per evaluator, or repeat runs
