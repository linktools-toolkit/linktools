...
"""Builtin tool execution and toolset construction — file and terminal tools."""
...
class HookedBuiltinToolset(WrapperToolset):
    ...
    MCP_CALL_START/POST_MCP_CALL hooks with `server="builtin"`, mirroring the
    attribution used by SkillCapability/SubagentCapability/HookedMCPCapability
    for their own tool categories.
    """

    def __init__(self, wrapped, hooks, trace_id: str, parent_call_id: str | None):
...
def build_builtin_toolset(context: BuiltinToolContext) -> FunctionToolset:
    ...
    if "file" in enabled_tools:
        async def list_dir(path: str = ".", recursive: bool = False) -> dict[str, Any]:
            ...
            return await run_file_tool("list_dir", {"path": path, "recursive": recursive}, runtime_dir, base_dirs)
        ...
        async def batch_files(operations: list[dict[str, Any]]) -> dict[str, Any]:
            ...
            return await run_file_tool("batch_files", {"operations": operations}, runtime_dir, base_dirs)

        for fn in (list_dir, read_file, write_file, batch_files):
    ...
    if "terminal" in enabled_tools:
        async def bash(command: str, timeout_ms: int | None = None) -> dict[str, Any]:
            ...
            resolved = command
            if base_dirs:
                resolved = await asyncio.to_thread(resolve_base_file_paths, command, base_dirs)
            args: dict[str, Any] = {"command": resolved}
            if timeout_ms is not None:
                args["timeout_ms"] = timeout_ms
            return await run_bash_tool(args, runtime_dir, timeout_s=60.0)
