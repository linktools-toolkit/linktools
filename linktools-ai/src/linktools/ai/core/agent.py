...
from .session import FileSessionSpec, Session, SessionTurn, build_input_display_entries
...
from .prompt import PromptContext, build_prompt, unavailable_adapters_note
from .runtime import AgentExecutionContext, AgentKernel
from ..capabilities.run import RuntimeRunCapability
from .skill_view import view_skill, view_available_skills
from .builtin_tools import BuiltinToolContext, HookedBuiltinToolset, build_builtin_toolset
...
class BaseAgent(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        self.kernel = execution_context.kernel if execution_context and execution_context.kernel is not None else AgentKernel(environ)
        self.execution_context = execution_context or self.kernel.build_context(
            spec,
            session,
...
class LlmAgent(BaseAgent):
    ...
    def _build_model_agent(self, request: LlmCallRequest) -> BuiltLlmAgent:
        ...
        bundle = build_model(self.environ, request.model_type)
        ...
        hooks = self.environ.hooks
        if hooks:
            builtin_toolset = HookedBuiltinToolset(builtin_toolset, hooks, self.session.trace_id, request.parent_call_id)
        ...
        capabilities: list[Any] = [runtime_capability]
        if request.skills:
            capabilities.append(SkillCapability(
                skill_view_fn=lambda arguments: self._read_skill(request.skills, arguments),
                hooks=hooks, trace_id=self.session.trace_id, parent_call_id=request.parent_call_id,
            ))
    ...
    async def _invoke_subagent(
        ...
        *,
        ...
        t = time.monotonic()
        if hooks:
            hooks.fire(HookEvent.SUBAGENT_START, trace_id=trace_id, parent_agent_id=self.agent_id, subagent_id=subagent_id, call_id=call_id)
        success = True
        try:
            child_spec = next(
            ...
            if hooks:
                hooks.fire(HookEvent.SUBAGENT_END, trace_id=trace_id, parent_agent_id=self.agent_id, subagent_id=subagent_id, duration_ms=round((time.monotonic() - t) * 1000, 2), status="completed" if success else "failed", call_id=call_id)
    ...
    @classmethod
    def from_spec(cls, spec: AgentSpec, environ: AgentEnvironment) -> "LlmAgent":
        """Construct a lightweight instance for capability inspection (no real session)."""
        session = Session.create(environ, FileSessionSpec(trace_id="", session_id="snapshot"))
        return cls(environ, spec, session)
...
class SubAgent(RuntimeAgent):
    ...
    def __init__(
        ...
        super().__init__(environ, spec, session, execution_context=execution_context)
