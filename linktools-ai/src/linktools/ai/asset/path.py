...
def asset_path(root: AssetRoot, key: AssetKey) -> Path:
    if root.scheme != "file" or not key.kind or not key.id:
        raise LinktoolsAIError(ErrorCode.ASSET_PATH_ABSOLUTE)
    if key.kind in _RESERVED_ROOT_NAMES:
        raise LinktoolsAIError(ErrorCode.ASSET_PATH_OUTSIDE_ROOT)
    ...
    try:
        ...
        raise LinktoolsAIError(ErrorCode.ASSET_PATH_OUTSIDE_ROOT) from exc
    return candidate
