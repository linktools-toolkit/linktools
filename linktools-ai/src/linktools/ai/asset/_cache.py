...
from ._content import AssetContent, AssetContentSource
...
class AssetObjectCache(Generic[TAsset]):
    ...
    async def list_ids(self) -> "tuple[str, ...]":
        ...
        infos = await self._source.list_info()
        ids = [
    ...
    async def get(self, item_id: str) -> TAsset:
        content = await self._source.get(self._full_path(item_id))
        if content is None:
...
__all__ = ["AssetCacheCodec", "AssetObjectCache"]
