...
class SqlAssetBackend:
    ...
    def __init__(
        ...
        *,
        ...
        self._tables = _build_tables(_load_sqlalchemy().MetaData())
    ...
    async def initialize(self) -> None:
        ...
        async with self._sessions() as session:
            async with session.begin():
                if await session.scalar(sqlalchemy.select(self._tables.root.c.id).where(self._tables.root.c.id == 1)) is None:
                    now = datetime.now(timezone.utc)
                    try:
                        async with session.begin_nested():
                            await session.execute(
                                sqlalchemy.insert(self._tables.root).values(
                                    id=1,
                                    revision=0,
                                    created_at=now,
                                    updated_at=now,
        ...
        async with self._sessions() as session:
            ...
            if after_revision is not None:
                ...
                if 0 <= previous < current:
                    rows = (
                        await session.execute(
                            sqlalchemy.select(self._tables.version)
    ...
    async def get(self, key: AssetKey) -> "bytes | None":
        ...
        async with self._sessions() as session:
            row = (
                await session.execute(
                    sqlalchemy.select(self._tables.entry).where(
    ...
    async def reset(self) -> "StorageResetResult[AssetStoreRevision]":
        ...
        async with self._sessions() as session:
            async with session.begin():
                ...
                rows = (
                    await session.execute(
                        sqlalchemy.select(self._tables.entry).where(self._tables.entry.c.active.is_(True))
                    )
                ).mappings().all()
                if not rows:
    ...
    async def _entry(self, session: "AsyncSession", key: AssetKey) -> "Mapping[str, SqlValue] | None":
        ...
        return (
            await session.execute(
                sqlalchemy.select(self._tables.entry).where(self._tables.entry.c.path == _path_for_key(key))
            )
        ).mappings().first()
    ...
    async def _current_revision(self, session: "AsyncSession") -> int:
        ...
        value = await session.scalar(
            sqlalchemy.select(self._tables.root.c.revision).where(self._tables.root.c.id == 1)
        )
    ...
    async def _reserve_revision(self, session: "AsyncSession", expected: int) -> int:
        ...
        result = await session.execute(
            sqlalchemy.update(self._tables.root)
            .where(
    ...
    async def _write_change(self, session: "AsyncSession", info: AssetInfo, object_id: "str | None", deleted: bool, now: datetime) -> None:
        sqlalchemy = _load_sqlalchemy()
        await session.execute(
            sqlalchemy.insert(self._tables.version).values(
    ...
    async def _read_blob(self, session: "AsyncSession", digest: str) -> "bytes | None":
        sqlalchemy = _load_sqlalchemy()
        row = (await session.execute(sqlalchemy.select(self._tables.blob.c.content).where(self._tables.blob.c.sha256 == digest))).first()
        if row is None:
            return None
        content = row[0]
...
def _build_tables(metadata: "MetaData") -> SqlAssetTables:
    sqlalchemy = _load_sqlalchemy()
    integer_id = sqlalchemy.BigInteger().with_variant(sqlalchemy.Integer, "sqlite")
    ...
    root = sqlalchemy.Table(
        root_name,
        metadata,
        sqlalchemy.Column("id", integer_id, primary_key=True, autoincrement=True),
        ...
        sqlalchemy.Index(storage_name("asset_revision_ix_revision"), "revision"),
        sqlalchemy.Index(storage_name("asset_revision_ix_updated_at"), "updated_at"),
        sqlalchemy.Index(storage_name("asset_revision_ix_created_at"), "created_at"),
    )
    entry = sqlalchemy.Table(
        ...
        sqlalchemy.Column("kind", sqlalchemy.String(128), nullable=False),
        sqlalchemy.Column("version", sqlalchemy.Integer, nullable=False),
        sqlalchemy.Column("etag", sqlalchemy.String(255), nullable=False),
        ...
        sqlalchemy.Column("updated_at", sqlalchemy.DateTime(timezone=True), nullable=False),
        sqlalchemy.Column("created_at", sqlalchemy.DateTime(timezone=True), nullable=False),
        sqlalchemy.UniqueConstraint("path", name=storage_name("asset_contents_uk_path")),
        sqlalchemy.Index(storage_name("asset_contents_ix_kind"), "kind"),
        sqlalchemy.Index(storage_name("asset_contents_ix_updated_at"), "updated_at"),
        sqlalchemy.Index(storage_name("asset_contents_ix_created_at"), "created_at"),
    )
    version = sqlalchemy.Table(
        ...
        sqlalchemy.Column("revision", sqlalchemy.Integer, nullable=False),
        sqlalchemy.Column("path", sqlalchemy.String(512), nullable=False),
        sqlalchemy.Column("kind", sqlalchemy.String(128), nullable=True),
        sqlalchemy.Column("version", sqlalchemy.Integer, nullable=True),
        sqlalchemy.Column("etag", sqlalchemy.String(255), nullable=True),
        sqlalchemy.Column("object_id", sqlalchemy.String(128), nullable=True),
        sqlalchemy.Column("active", sqlalchemy.Boolean, nullable=True),
        sqlalchemy.Column("deleted", sqlalchemy.Boolean, nullable=False),
        sqlalchemy.Column("updated_at", sqlalchemy.DateTime(timezone=True), nullable=False),
        sqlalchemy.Column("created_at", sqlalchemy.DateTime(timezone=True), nullable=False),
        sqlalchemy.UniqueConstraint("revision", "path", name=storage_name("asset_changes_uk_revision_path")),
        sqlalchemy.Index(storage_name("asset_changes_ix_object_id"), "object_id"),
        sqlalchemy.Index(storage_name("asset_changes_ix_updated_at"), "updated_at"),
        sqlalchemy.Index(storage_name("asset_changes_ix_created_at"), "created_at"),
    )
    blob = sqlalchemy.Table(
        blob_name,
        metadata,
        sqlalchemy.Column("id", integer_id, primary_key=True, autoincrement=True),
        ...
        sqlalchemy.Column("updated_at", sqlalchemy.DateTime(timezone=True), nullable=False),
        sqlalchemy.Column("created_at", sqlalchemy.DateTime(timezone=True), nullable=False),
        sqlalchemy.UniqueConstraint("sha256", name=storage_name("asset_blobs_uk_sha256")),
        sqlalchemy.Index(storage_name("asset_blobs_ix_updated_at"), "updated_at"),
        sqlalchemy.Index(storage_name("asset_blobs_ix_created_at"), "created_at"),
    )
