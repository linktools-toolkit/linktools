...
"""Plain filesystem-directory asset backend."""
...
logger = environ.get_logger("ai.asset.persistence.local")
...
class LocalAssetBackend:
    ...
    def _resolve(self, path: str) -> Path:
        ...
        try:
            ...
            raise AssetConflictError(f"asset path escapes root: {path!r}") from exc
        return target

    async def initialize_storage(self) -> None:
        await asyncio.to_thread(self.root.mkdir, parents=True, exist_ok=True)
        logger.debug("initialized local asset storage: root=%s", self.root)
    ...
    async def put(self, entry: AssetContent) -> "tuple[AssetContent, None]":
        ...
        logger.info("stored local asset: path=%s", entry.info.path)
        return entry, None

    async def delete(self, path: str) -> None:
        await asyncio.to_thread(_unlink_if_exists, self._resolve(path))
        logger.info("deleted local asset: path=%s", path)
...
def _unlink_if_exists(target: Path) -> None:
    try:
        ...
        logger.warning("failed to delete local asset file: path=%s error=%s", target, exc)


__all__ = ["LocalAssetBackend", "PrefixAssetPathAdapter", "AssetPathAdapter"]
