...
class ReminderMiddleware(Middleware):
    def __init__(self, *, max_messages: int = 80, threshold_ratio: float = 0.7, reminder_text: str) -> None:
    ...
    async def before_model(self, context: Any, request: Any) -> Any:
        if not self._already_reminded and len(request.messages) / self._max_messages >= self._threshold_ratio:
