...
* ``name`` -- a project-level agent (resolved through the project AgentSpecIndex);
...
:mod:`linktools.ai.agent.skill.private` (path/symlink rejection, parsing) and an
...
class _ProjectAgentSource(Protocol):
    """Anything that returns an AgentSpec by name (the project AgentSpecIndex
