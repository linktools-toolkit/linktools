#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""LocalSandbox: subprocess + direct file IO on the local filesystem.
...
class LocalSandbox:
    ...
    async def fork(self, branch_dir: Path) -> "LocalSandbox":
        ...
        return LocalSandbox(runtime_dir=branch_dir, base_dirs=self.base_dirs)
...
async def run_bash_tool(
    ...
    # Register the live proc so LocalSandbox.terminate() can reap it
...
def resolve_base_file_paths(command: str, base_dirs: "list[Path]") -> str:
    for base_dir in base_dirs:
        ...
        for file_path in sorted(files, key=lambda p: len(p.parts), reverse=True):
            ...
            # back to the source feature tree; otherwise relative_to(base_dir) fails
            # for builtin files materialized as symlinks under workspace/TRC.../features.
...
def _strip_patch_prefix(path: str) -> str:
    ...
    # Verified empirically: a header like "zzz/features/x.txt" is written
    # by real `patch -p1` to "features/x.txt", stripping "zzz/" just like
...
def _resolve_runtime_path(rel_path: str, runtime_dir: Path) -> Path:
    ...
    for forbidden in ("base/", "features/"):
        if path_text == forbidden.rstrip("/") or path_text.startswith(forbidden):
            raise ValueError(
                f"write_file cannot write to feature/base paths: {rel_path}"
            )
