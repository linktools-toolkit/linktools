...
``WorkspaceManager`` is the Protocol every backend satisfies; it accepts only
the workspace identity fields and does not depend on the execution domain."""
...
class WorkspaceManager(Protocol):
    """Resolve a WorkspaceRef to a physical execution directory.
...
class LocalWorkspaceManager:
    ...
    async def resolve(self, workspace: WorkspaceRef) -> Workspace:
        ...
        return Workspace(ref=workspace, root=path)
