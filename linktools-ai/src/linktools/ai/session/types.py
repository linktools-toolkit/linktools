...
class Session(abc.ABC):
    ...
    @classmethod
    def create(cls, workspace_root: Path, trace_root: Path, spec: FileSessionSpec) -> "FileSession":
        """Create a filesystem-backed session (pipeline / runtime default)."""
        return FileSession.create(workspace_root, trace_root, spec)

    @classmethod
    def create_db(cls, workspace_root: Path, trace_root: Path, spec: RemoteSessionSpec) -> "Session":
        """Create a DB-backed session through the unified Session factory."""
        return RemoteSession.create(workspace_root, trace_root, spec)
...
class RemoteSession(Session):
    ...
    @classmethod
    def create(cls, workspace_root: Path, trace_root: Path, spec: RemoteSessionSpec) -> "RemoteSession":
        trace_id = spec.trace_id or spec.session_id
        resolved_trace_root = spec.trace_root if spec.trace_root is not None else trace_root
        return cls(
            ...
            run=RunContext(
                ...
                runtime_dir=resolved_trace_root / "runtime",
                session_dir=resolved_trace_root / "session" / spec.session_id,
            ),
            store=spec.store,
            artifact_store=spec.artifact_store or LocalArtifactStore(),
            status_store=spec.status_store,
