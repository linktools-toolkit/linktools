...
from ...agent.capability.models import CapabilityRef
from ...errors import (
...
class MCPProvider:
    ...
    async def resolve(
        ...
        for server_id in ids:
            ...
            if not discovery.verified and discovery_mode == "best_effort":
                ...
                await context.security_event_emitter.emit_security(
                    SecurityDegraded(
            ...
            from ..models import ToolContribution, ManagedToolDefinition
    ...
    async def _target_ids(
        self, ref: CapabilityRef, context: CapabilityContext
    ) -> "tuple[str, ...]":
        if ref.name == "*":
