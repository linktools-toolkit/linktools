...
Filesystem + SqlAlchemy backends provide. The base BlobStore / ArtifactRecordStore
Protocols are intentionally minimal (not every backend can list its blobs); a
backend is sweepable only if it implements that extension.
"""
...
async def sweep_orphan_blobs(
    ...
    *,
    ...
    async for digest_value, modified_at in blob_store.iter_digests_with_mtime():
        ...
        # snapshot.
        async with coordinator.hold(digest.value):
            # Re-stat: the blob may have been deleted since the scan.
            if await blob_store.stat(digest=digest.value) is None:
                continue
            ...
            if referenced:
                ...
                continue
            try:
                await blob_store.delete(digest=digest.value)
            except Exception:
                if metrics is not None:
