#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""RuntimeTaskHandler: drive a Run as a task via an explicitly-injected
:class:`TaskRunDispatcher` (in production, a ``linktools.ai.Runtime``).
...
from ...identity.principal import ActorRef, PrincipalContext, ScopeSet


class TaskRunDispatcher(Protocol):
    """The narrow slice of Runtime/RunCoordinator's surface RuntimeTaskHandler
    needs: run a raw spec+prompt end-to-end (session resolution, compile,
    dispatch) and cancel an in-flight run. Distinct from
    ``run.dispatch.RunDispatcher`` (which operates post-compile, on an
    already-compiled agent, for Swarm/Subagent execution) -- this Protocol
    covers the raw-spec-in, full-pipeline entry point that
    ``RunCoordinator.run``/``RunCoordinator.cancel`` (and ``Runtime``, which
    delegates to them) already provide."""
...
class RuntimeTaskHandler:
    ...
    async def execute(
        ...
        try:
            bind_runnable = getattr(self._task_store, "bind_runnable", None)
            if bind_runnable is not None:
                await bind_runnable(
    ...
    async def _run_with_cancellation(
        ...
        """Drive ``dispatcher.run`` while racing the task's cancellation token.
        ...
        first, the run is stopped via ``dispatcher.cancel`` and given a grace
        ...
        try:
            await self._dispatcher.cancel(run_id, principal=task_principal)
