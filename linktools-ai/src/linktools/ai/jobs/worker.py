#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""JobWorker: the claim-execute-commit loop.
...
from .runtime import JobRuntimeOptions
from ..governance.security.redact import redact_exception, redact_text
from .store import TaskClaimLostError, JobStore, claim_matches_task
...
class JobWorker:
    def __init__(
        ...
        *,
        ...
        artifact_store=None,
        observability_metrics: "Any | None" = None,
    ) -> None:
                ...
                if (self._clock.now() - last_recover).total_seconds() >= recover_every:
                    ...
                    if recovered:
                        ...
                        if self._obs_metrics is not None:
                            # Each recovered task represents a lease that
                            # expired underneath its former owner. Count once
                            # per recovery batch (the lease-expiry signal) and
                            # once per recovered task (the recovery signal);
                            # both are bounded and operator-facing.
    ...
    async def _execute(self, claimed, claimed_at) -> None:
        ...
        heartbeat = asyncio.create_task(
            self._heartbeat(claim, cancellation, task.lease_expires_at)
        )
        try:
    ...
    async def _heartbeat(
        ...
        * when the renewed task reads back CANCELLING/CANCELLED (a
        ...
        # gone locally. Seeded from the just-claimed task's lease expiry -- the
        # claim was made immediately before the heartbeat started, so its lease
        # runs out roughly one lease interval from here.
        confirmed_lease_expires_at = initial_lease_expires_at
        while True:
            await self._clock.sleep(self._options.heartbeat_seconds)
            try:
                ...
                if self._obs_metrics is not None:
                    # The worker's fence no longer matches the stored task --
                    # either the lease was reclaimed by another worker or the
                    # fencing token was superseded. Both fall under "stale
                    # fence" for the observability floor.
                    self._obs_metrics.counter("job_stale_fence_total")
                cancellation.trigger()
                return
