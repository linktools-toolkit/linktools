...
from linktools.ai.storage.sqlalchemy.models import RunRow
...
def _row_to_record(row: RunRow) -> RunRecord:
    # data_json envelope carries input/result/error/cancel_reason (the >2-text
    # lint rule forced these out of separate columns); metadata stays its own
    # column so it can be updated independently.
    ...
    return RunRecord(
        ...
        input=RunInput(**data["input"]),
        result=None if data.get("result") is None else RunResult(**data["result"]),
        error=None if data.get("error") is None else RunErrorInfo(**data["error"]),
        version=row.version,
...
class SqlAlchemyRunStore:
    ...
    async def get(self, run_id: str) -> "RunRecord | None":
        async def _do(session):
            result = await session.execute(select(RunRow).where(RunRow.run_id == run_id))
    ...
    async def claim_execution(self, run_id: str, *, worker_id: str, execution_token: str) -> RunRecord:
        ...
        async def _do(session):
            stmt = update(RunRow).where(RunRow.run_id == run_id).where(
    ...
    async def heartbeat_execution(self, run_id: str, *, worker_id: str, execution_token: str) -> RunRecord:
        ...
        async def _do(session):
            result = await session.execute(update(RunRow).where(
                RunRow.run_id == run_id, RunRow.worker_id == worker_id,
    ...
    async def transition(
        ...
        *,
        ...
        async def _do(session):
            # Read the current row to merge result/error/cancel_reason into the
            # data_json envelope (input must be preserved across the update).
            # The UPDATE's WHERE version = :expected clause below remains the
            # atomic race-decider; this read only supplies the envelope.
            row = (
            ...
            # request (set on entering CANCELLING/CANCELLED via a Principal).
            ...
            if result_proxy.rowcount == 0:
                ...
                if fresh.version != expected_version:
                    raise RunConflictError(
                        f"expected version {expected_version}, found {fresh.version}"
                    )
                current_status = RunStatus(fresh.status)
