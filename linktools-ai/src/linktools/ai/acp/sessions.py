    ...
    async def toolsets(self) -> "tuple[Any, ...]":
        ...
        try:
            ...
            for descriptor in self.descriptors:
                values.append(await pool.get_toolset(_mcp_spec(descriptor)))
        except Exception:
            await pool.close()
...
def validate_session_paths(
    *, project_root: "str | Path", cwd: str, additional_directories: "list[str] | None"
    ...
    return str(target), tuple(dict.fromkeys(additional))
...
class AcpSessionService:
    ...
    async def load_or_resume(
        ...
        *,
        ...
        async with active.lock:
            if active.record.cwd != normalized_cwd or active.record.additional_directories != directories:
            ...
            record = replace(active.record, closed=False, updated_at=datetime.now(timezone.utc))
            self.repository.save(record)
            active.record = record
            await active.mcp_resources.close()
    ...
    async def fork(self, source_session_id: str, *, cwd: str, additional_directories: "list[str] | None", mcp_servers: "list[Any] | None") -> ActiveAcpSession:
        ...
        async with source.lock:
            ...
            _validate_mcp_descriptors(mcp_servers or ())
            fingerprints = tuple(sorted(mcp_descriptor_fingerprint(server) for server in (mcp_servers or ())))
            if fingerprints != source.record.mcp_server_fingerprints:
            ...
            record = AcpSessionRecord(
                ...
                cwd=normalized_cwd,
                additional_directories=directories,
                mode_id=source.record.mode_id,
                config_values=dict(source.record.config_values),
    ...
    async def close(self, session_id: str) -> None:
        ...
        async with active.lock:
            if active.active_execution_id is not None:
                try:
                    await self.runtime.cancel(active.active_execution_id, principal=self.principal)
                except Exception:
                    pass
    ...
    async def set_mode(self, session_id: str, mode_id: str) -> ActiveAcpSession:
        ...
        async with active.lock:
            ...
            active.record = replace(active.record, mode_id=mode_id, updated_at=datetime.now(timezone.utc))
    ...
    async def set_config(self, session_id: str, config_id: str, value: Any) -> ActiveAcpSession:
        ...
        async with active.lock:
            ...
            active.record = replace(active.record, config_values=values, updated_at=datetime.now(timezone.utc))
            self.repository.save(active.record)
            return active
    ...
    async def _assert_complete_history(self, session_id: str) -> None:
        ...
        for view in views:
            if getattr(view, "status", None) is not RunStatus.COMPLETED or getattr(view, "capture_state", None) != "complete":
...
__all__ = ["ActiveAcpSession", "AcpSessionService", "SessionMcpResources", "validate_session_paths"]


def _mcp_spec(descriptor: Any) -> Any:
    """Convert one official ACP descriptor to the existing MCP domain type."""
    from ..agent.mcp.spec import MCPServerSpec

    kind = getattr(descriptor, "type", None) or "stdio"
    ...
    raise request_error("unsupported_mcp_transport")


def _validate_mcp_descriptors(descriptors: Any) -> None:
    try:
        for descriptor in descriptors:
            _mcp_spec(descriptor)
