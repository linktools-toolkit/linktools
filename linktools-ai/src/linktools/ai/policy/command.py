...
class CommandRule:
    def __init__(self, *, denied_patterns: "tuple[str, ...]" = DEFAULT_DENIED_COMMAND_PATTERNS) -> None:
    ...
    async def evaluate(self, request: ToolRequest, context: ToolContext) -> PolicyDecision:
        ...
        if request.category is not None:
            ...
            # never threads descriptor info through falls back to the legacy
        ...
        for pattern in self._compiled:
            if pattern.search(command):
                return PolicyDecision(
                    kind=PolicyDecisionKind.DENY, rule_id="command-rule",
