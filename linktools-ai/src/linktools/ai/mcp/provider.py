...
refs. Resolves server specs via an MCPServerSpecProvider and
...
- enabled_tools / disabled_tools filtering, cross-server conflict detection,
  ...
  cannot be applied or verified without a live MCP server -- live
  stdio/sse/http servers are environment-gated acceptance criteria. The
...
class MCPProvider:
    ...
    async def _target_ids(self, ref: CapabilityRef, context: CapabilityContext) -> "tuple[str, ...]":
        if ref.name == "*":
            ...
            # only. The Runtime gate is authoritative; a tool
