...
class DefaultEventService:
    ...
    def __init__(self, persistence: RuntimePersistence, authorization: AuthorizationPolicy) -> None:
    ...
    async def stream(self, execution_id: str, *, principal: Principal, after_sequence: int = 0) -> AsyncIterator[ExecutionStreamItem]:
        ...
        while True:
            ...
            if not page.items:
                execution = await self._persistence.executions.get(execution_id, tenant_id=principal.tenant_id)
