...
class DefaultArtifactService:
    ...
    async def list(self, execution_id: str, *, principal: Principal, cursor: "str | None" = None, limit: int = 100) -> Page[ArtifactView]:
        ...
        page = await self._persistence.artifact.list_by_execution(execution_id, tenant_id=principal.tenant_id, cursor=raw_cursor, limit=limit)
    ...
    async def get(self, artifact_id: str, *, principal: Principal) -> ArtifactDownload:
        header = await self._persistence.artifact.get_header(artifact_id, tenant_id=principal.tenant_id)
        if header is None:
            raise AIError(ErrorCode.AUTHORIZATION_DENIED)
        await self._authorization.authorize(principal, AuthorizationAction.ARTIFACT_READ, header)
        record = await self._persistence.artifact.get_metadata(artifact_id, tenant_id=principal.tenant_id)
        if record is None:
            raise AIError(ErrorCode.AUTHORIZATION_DENIED)
        ...
        operation = await self._persistence.operation_ledger.append(OperationLedgerInput(nonce, principal.tenant_id, ResourceKind.DOWNLOAD_GRANT, artifact_id, record.execution_id, OperationKind.DOWNLOAD_GRANT, OperationStatus.PENDING, request_digest, record.blob_ref, record.digest, None, True, now, now))
    ...
    async def verify_grant(self, token: str, *, principal: Principal) -> str:
        try:
            ...
            operation = await self._persistence.operation_ledger.get(str(payload["nonce"]), tenant_id=principal.tenant_id)
            ...
            record = await self._persistence.artifact.get_metadata(str(payload["artifact_id"]), tenant_id=principal.tenant_id)
