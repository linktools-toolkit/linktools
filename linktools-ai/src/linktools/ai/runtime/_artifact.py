...
class DefaultArtifactService:
    ...
    async def get(self, artifact_id: str, *, principal: Principal) -> ArtifactDownload:
        ...
        request_digest = canonical_sha256({"action": "artifact.download", "tenant_id": principal.tenant_id, "principal_id": principal.principal_id, "artifact_id": artifact_id, "artifact_digest": record.object_ref.digest})
        now = datetime.now(timezone.utc)
        operation = await self._state.operations.append(OperationLedgerInput(nonce, principal.tenant_id, ResourceKind.DOWNLOAD_GRANT, artifact_id, record.execution_id, OperationKind.DOWNLOAD_GRANT, OperationStatus.PENDING, request_digest, record.object_ref.key, record.object_ref.digest, None, True, now, now))
        payload = {"tenant_id": principal.tenant_id, "principal_id": principal.principal_id, "artifact_id": artifact_id, "artifact_digest": record.object_ref.digest, "expires_at": expires_at, "nonce": nonce}
    ...
    async def verify_grant(self, token: str, *, principal: Principal) -> str:
        try:
            ...
            if record is None or record.object_ref.digest != artifact_digest:
