...
class DefaultArtifactService:
    ...
    def __init__(self, persistence: RuntimePersistence, authorization: AuthorizationPolicy, *, grant_key: bytes, cursor_signer: CursorSigner, entry_path: str = "/v1/artifacts") -> None:
        if not grant_key:
    ...
    async def list(self, execution_id: str, *, principal: Principal, cursor: "str | None" = None, limit: int = 100) -> Page[ArtifactView]:
        ...
        page = await self._persistence.artifacts.list_by_execution(execution_id, tenant_id=principal.tenant_id, cursor=raw_cursor, limit=limit)
    ...
    async def verify_grant(self, token: str, *, principal: Principal) -> str:
        try:
            ...
            operation = await self._persistence.operations.get(str(payload["nonce"]), tenant_id=principal.tenant_id)
            ...
            record = await self._persistence.artifacts.get_metadata(str(payload["artifact_id"]), tenant_id=principal.tenant_id)
