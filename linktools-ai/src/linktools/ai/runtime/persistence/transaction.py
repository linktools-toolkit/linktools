...
:mod:`linktools.ai.runtime.persistence.protocols`; this module holds the
in-repo reference managers. ``storage.transaction()`` is the single public
cross-store-UoW entry: it yields a :class:`StorageUnitOfWork` whose stores
share one atomic scope (a SqlAlchemy AsyncSession + its transaction). The
manager object itself is an internal Storage dependency
(``_transaction_manager``), not a public surface; callers go through
``storage.transaction()``.

A backend whose stores are independent (FilesystemStorage -- separate file
backends, no shared transaction provider) CANNOT offer a cross-store UoW;
its ``features.transaction_scope`` is ``TransactionScope.NONE`` (each store
is independently durable, but there is no general cross-store transaction),
and its manager is a :class:`NoCrossStoreTransactions` whose
...
``async with storage.transaction()`` fails at the call with the concrete
error, not with a confusing coroutine TypeError.
"""
