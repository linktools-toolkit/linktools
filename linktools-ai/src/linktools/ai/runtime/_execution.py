...
class DefaultExecutionService:
    ...
    async def _start_unlocked(self, binding_digest: str, request: ExecutionRequest, *, session_id: "str | None" = None, source_execution_id: "str | None" = None, base_execution_id: "str | None" = None, parent_execution_id: "str | None" = None, root_execution_id: "str | None" = None, lineage_kind: ExecutionLineageKind = ExecutionLineageKind.RUN, scope: str = "execution.run") -> ExecutionHandle:
        ...
        if session_id is not None and source_execution_id is None:
            session = await self._persistence.conversation.get(session_id, tenant_id=request.principal.tenant_id)
            if session is None:
        ...
        existing = await self._persistence.execution_idempotency.get(scope, key_hash, tenant_id=request.principal.tenant_id)
        if existing is not None:
            ...
            if existing.status is IdempotencyStatus.RESERVED:
                pending = await self._persistence.execution.get(existing.execution_id, tenant_id=request.principal.tenant_id)
                ...
                try:
                    await self._persistence.execution.claim_start(ExecutionStartClaim(existing.execution_id, request.principal.tenant_id, pending.revision, pending.event_sequence, scope, key_hash, request_digest, datetime.now(timezone.utc)))
                except AIError as error:
                    current = await self._persistence.execution.get(existing.execution_id, tenant_id=request.principal.tenant_id)
        ...
        if session_id is not None:
            active = await self._persistence.execution.list_by_session(session_id, tenant_id=request.principal.tenant_id)
        ...
        reservation = await self._persistence.execution.reserve_start(
            ExecutionStartReservation(
                execution,
        ...
        if not reservation.created:
            if reservation.execution_idempotency.request_digest != request_digest:
                raise AIError(ErrorCode.IDEMPOTENCY_CONFLICT)
            if reservation.execution.status is ExecutionStatus.PENDING_START and reservation.execution_idempotency.status is IdempotencyStatus.RESERVED:
                try:
                    await self._persistence.execution.claim_start(ExecutionStartClaim(reservation.execution.execution_id, reservation.execution.tenant_id, reservation.execution.revision, reservation.execution.event_sequence, scope, key_hash, request_digest, now))
                except AIError as error:
                    current = await self._persistence.execution.get(reservation.execution.execution_id, tenant_id=reservation.execution.tenant_id)
                    if error.code is not ErrorCode.STORAGE_CONFLICT or current is None or current.status is not ExecutionStatus.STARTED:
                        raise
            ...
            elif reservation.execution.status is ExecutionStatus.STARTED and reservation.execution_idempotency.status is IdempotencyStatus.STARTED:
                return ExecutionHandle(reservation.execution.execution_id)
            elif reservation.execution.status is ExecutionStatus.START_UNKNOWN or reservation.execution_idempotency.status is IdempotencyStatus.START_UNKNOWN:
                raise AIError(ErrorCode.EXECUTION_START_UNKNOWN)
            else:
                raise AIError(ErrorCode.STORAGE_INTEGRITY_ERROR)
            return ExecutionHandle(reservation.execution.execution_id)
        ...
        try:
            await self._persistence.execution.claim_start(
    ...
    async def _launch_claim(self, request: ExecutionRequest, execution_id: str, tenant_id: str, scope: str, key_hash: str) -> None:
        if self._backend is None:
            raise AIError(ErrorCode.RUNTIME_DEPENDENCY_NOT_READY)
        try:
            launch_record = await self._persistence.execution.get(execution_id, tenant_id=tenant_id)
            ...
            current = await self._persistence.execution.get(execution_id, tenant_id=tenant_id)
            if current is not None and current.status is ExecutionStatus.STARTED:
                identity = await self._persistence.execution_idempotency.get(scope, key_hash, tenant_id=tenant_id)
                if identity is None:
                    raise AIError(ErrorCode.STORAGE_INTEGRITY_ERROR)
                await self._persistence.execution.mark_start_unknown(ExecutionStartUnknownCommit(execution_id, tenant_id, current.revision, current.event_sequence, scope, key_hash, identity.request_digest, datetime.now(timezone.utc)))
    ...
    async def result(self, execution_id: str, *, principal: Principal) -> ExecutionResult:
        ...
        result = await self._persistence.execution_result.get(execution_id, tenant_id=principal.tenant_id)
        if result is None:
            raise AIError(ErrorCode.STORAGE_INTEGRITY_ERROR)
        ...
        if not result.payload_ref or not result.payload_digest:
            raise AIError(ErrorCode.STORAGE_INTEGRITY_ERROR)
        try:
            payload = await BlobPayloadService(self._persistence.blobs.for_domain(StorageDomain.EXECUTION), tenant_id=principal.tenant_id).load(PayloadRef(result.payload_ref))
    ...
    async def cancel(self, execution_id: str, request: CancelExecutionRequest) -> CancelExecutionResult:
        ...
        operation = await self._persistence.operation_ledger.get(
            operation_id,
            tenant_id=request.principal.tenant_id,
        )
        if operation is not None:
            ...
            operation = await self._persistence.operation_ledger.append(operation)
        ...
        try:
            try:
                cancelling = await self._persistence.execution.request_cancel(
                    ExecutionCancelRequestCommit(
                        execution_id=execution_id,
                        tenant_id=request.principal.tenant_id,
            ...
            if outcome is CancelEffectOutcome.UNKNOWN:
                ...
                if resolved is not None:
                    return resolved
                current = await self._persistence.operation_ledger.get(operation.operation_id, tenant_id=request.principal.tenant_id)
                if current is None:
                    raise AIError(ErrorCode.STORAGE_INTEGRITY_ERROR)
                if current.status is OperationStatus.PENDING:
                    try:
                        current = await self._persistence.operation_ledger.compare_and_swap(
                ...
                if current.status in {OperationStatus.SUCCEEDED, OperationStatus.CANCELLED}:
                    latest = await self._persistence.execution.get(execution_id, tenant_id=request.principal.tenant_id)
                ...
                raise AIError(ErrorCode.STORAGE_CONFLICT)
            cancelling_current = await self._persistence.execution.get(execution_id, tenant_id=request.principal.tenant_id)
            ...
            idempotency_records = await self._persistence.execution_idempotency.list_by_execution(execution_id, tenant_id=request.principal.tenant_id)
            ...
            current_operation = await self._persistence.operation_ledger.get(operation.operation_id, tenant_id=request.principal.tenant_id)
            ...
            try:
                ...
                if current is not None and current.status is OperationStatus.PENDING:
                    await self._persistence.operation_ledger.compare_and_swap(
                        operation.operation_id,
                        tenant_id=request.principal.tenant_id,
                        expected_status=OperationStatus.PENDING,
    ...
    async def _resolve_cancel_race(
        ...
        if current is None or current.status not in {ExecutionStatus.SUCCEEDED, ExecutionStatus.FAILED, ExecutionStatus.CANCELLED}:
            return None
        current_operation = await self._persistence.operation_ledger.get(operation.operation_id, tenant_id=tenant_id)
        if current_operation is None:
            raise AIError(ErrorCode.STORAGE_INTEGRITY_ERROR)
        for attempt in range(2):
            ...
            if current_operation.status not in {OperationStatus.PENDING, OperationStatus.EFFECT_UNKNOWN}:
                raise AIError(ErrorCode.STORAGE_CONFLICT)
            try:
                current_operation = await self._persistence.operation_ledger.compare_and_swap(
                    operation.operation_id,
                    tenant_id=tenant_id,
                ...
                if error.code is not ErrorCode.STORAGE_CONFLICT:
                    raise
                reloaded = await self._persistence.operation_ledger.get(operation.operation_id, tenant_id=tenant_id)
                if reloaded is None:
                    raise AIError(ErrorCode.STORAGE_INTEGRITY_ERROR)
    ...
    async def _load_authorized(self, execution_id: str, principal: Principal, action: AuthorizationAction) -> ExecutionRecord:
        ...
        record = await self._persistence.execution.get(execution_id, tenant_id=principal.tenant_id)
        if record is None:
            raise AIError(ErrorCode.AUTHORIZATION_DENIED)
        return record
