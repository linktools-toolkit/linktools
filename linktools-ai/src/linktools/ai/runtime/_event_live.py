#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Bounded process-local execution event streaming."""
...
from ..core import ExecutionDeltaType, JsonValue
...
class _LiveSubscription:
    ...
    def put_delta(self, value: ExecutionDelta) -> bool:
        ...
        while (
            self._queue_delta_count >= _QUEUE_LIMIT
            or self._queue_bytes + value_size > self._max_bytes
        ):
            if not self._drop_oldest_delta():
                break
        truncated = value.stream_truncated or self._truncated_pending
        self._truncated_pending = False
        value = ExecutionDelta(
            value.execution_id,
            value.delta_type,
            value.content,
            truncated,
...
class LiveExecutionEventBroker:
    ...
    def publish(self, delta: ExecutionDelta) -> None:
        ...
        if self._last_type.get(execution_id) is delta.delta_type and buffer:
            ...
            self._buffer_bytes[execution_id] = (
                self._buffer_bytes.get(execution_id, 0) + size
            )
    ...
    def _drop_oldest_delta(
        self,
        buffer: deque[_OrderedItem],
        execution_id: str,
    ) -> bool:
        for index, value in enumerate(buffer):
            if isinstance(value, ExecutionDelta):
                ...
                self._buffer_bytes[execution_id] -= len(
                    removed.content.encode("utf-8")
                )
...
__all__ = ["ExecutionDelta", "LiveExecutionEventBroker"]
