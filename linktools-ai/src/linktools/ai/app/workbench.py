...
class WorkspaceExecutionLauncher:
    ...
    def __init__(self, workspace: Workspace, runner: WorkspaceAgentRunner, stores: RuntimeStores) -> None:
    ...
    async def load_result(self, execution_id: str, *, tenant_id: str) -> WorkspaceAgentResult:
        record = await self._stores.domain.executions.get(execution_id, tenant_id=tenant_id)
        if record is None:
            raise AIError(ErrorCode.STORAGE_NOT_FOUND)
        if record.status not in {ExecutionStatus.SUCCEEDED, ExecutionStatus.FAILED, ExecutionStatus.CANCELLED}:
            raise AIError(ErrorCode.EXECUTION_START_UNKNOWN)
        result = await self._stores.domain.results.get(execution_id, tenant_id=tenant_id)
        if result is None:
            raise AIError(ErrorCode.STORAGE_NOT_FOUND)
        output = ""
        if result.payload_ref is not None:
            blob = await self._stores.domain.blobs.stat(
                BlobRef(tenant_id, result.payload_ref, 0, ""),
                tenant_id=tenant_id,
            )
            if blob is not None:
                chunks = bytearray()
                async for chunk in self._stores.domain.blobs.open(blob, tenant_id=tenant_id):
                    chunks.extend(chunk)
    ...
    async def _durable_run_id(self, record: ExecutionRecord, *, tenant_id: str) -> str:
        after_sequence = 0
        while True:
            page = await self._stores.domain.events.list(
                record.execution_id,
                tenant_id=tenant_id,
    ...
    async def reconcile(self) -> None:
        ...
        sessions = await self._stores.domain.sessions.list(tenant_id=self._workspace.workspace_id)
        for session in sessions:
            executions = await self._stores.domain.executions.list_by_session(session.session_id, tenant_id=session.tenant_id)
            for execution in executions:
                ...
                if await self._stores.steps.get_run(run_id=run_id) is not None:
                    continue
                current = await self._stores.domain.executions.get(execution.execution_id, tenant_id=execution.tenant_id)
    ...
    async def _history(self, execution: ExecutionRecord) -> list[object]:
        if execution.base_execution_id is None:
            return []
        base = await self._stores.domain.executions.get(execution.base_execution_id, tenant_id=execution.tenant_id)
        ...
        try:
            session = await self._stores.domain.sessions.get(execution.session_id or "", tenant_id=execution.tenant_id)
            if execution.lineage_kind is ExecutionLineageKind.FORK or (session is not None and base.session_id != session.session_id):
                return list(await fork_run(self._stores.steps, run_id=base_run_id))
            return list(await continue_run(self._stores.steps, run_id=base_run_id))
    ...
    async def _commit_success(self, execution: ExecutionRecord, result: WorkspaceAgentResult) -> None:
        now = datetime.now(timezone.utc)
        blob = await self._stores.domain.blobs.put_bytes(tenant_id=execution.tenant_id, data=result.output.encode("utf-8"))
        current = await self._stores.domain.executions.get(execution.execution_id, tenant_id=execution.tenant_id)
        if current is None:
    ...
    async def _terminal_idempotency(self, execution: ExecutionRecord, status: IdempotencyStatus, result_digest: str | None, error_code: str | None) -> IdempotencyTerminalUpdate | None:
        records = await self._stores.domain.idempotency.list_by_execution(execution.execution_id, tenant_id=execution.tenant_id)
        if len(records) > 1:
            raise AIError(ErrorCode.STORAGE_INTEGRITY_ERROR)
        if not records:
            return None
...
class WorkspaceAgentRuntime:
    ...
    def __init__(self, workspace: Workspace, *, runner: WorkspaceAgentRunner, stores: RuntimeStores, services: RuntimeServices, launcher: WorkspaceExecutionLauncher) -> None:
