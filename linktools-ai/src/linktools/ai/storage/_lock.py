...
from ._files import write_bytes_atomic
...
class KeyedAsyncLock:
    ...
    async def acquire(self, key: str) -> None:
        ...
        owner = cast("asyncio.Task[object]", task)
...
class FilesystemLeaseCoordinator:
    ...
    async def acquire(self, key: str, *, timeout: float = 30.0) -> Lease:
        ...
        try:
            ...
            try:
                await asyncio.wait_for(self._locks.acquire(key), timeout=remaining)
            ...
            if not attempt_cleanup_detached and lock_acquired:
...
def _lease_guard(
    ...
    *,
    ...
    lock = FileLock(str(guard_path), thread_local=False)
...
def _next_fence(path: Path, *, timeout: float | None = None) -> int:
    path.parent.mkdir(parents=True, exist_ok=True)
    lock = FileLock(str(path) + ".lock", thread_local=False)
    lock.acquire(timeout=-1 if timeout is None else timeout)
    try:
        fence = _read_fence(path) + 1
        write_bytes_atomic(path, str(fence).encode("utf-8"), fsync=True)
        return fence
    finally:
        lock.release()
