#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""FileRunCommitCoordinator: journaled cross-store commit for File backends.
...
class FileRunCommitCoordinator:
    ...
    async def _append_critical_event_once(
        ...
        if await self._event_exists(context, commit_id, type(payload).__name__):
            return
        await append_event(
            self._events,
            context,
    ...
    async def _event_exists(
        self, context: EventContext, commit_id: str, payload_type: str
    ) -> bool:
        """True if a critical event of ``payload_type`` tagged with ``commit_id``
        ...
        return any(
            ...
            and type(event.payload).__name__ == payload_type
