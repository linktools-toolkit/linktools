...
from ...identity.principal import ScopeSet

class FilesystemTaskCommitRecoveryError(RuntimeError):
...
class FilesystemTaskCommitJournal:
    ...
    def mark_step(self, path: Path, step: str) -> None:
        ...
        if step not in order or current not in order or order[step] < order[current]:
            raise FilesystemTaskCommitRecoveryError(
    ...
    def ensure_step(self, path: Path, step: str) -> bool:
        ...
        if current not in order or step not in order:
            raise FilesystemTaskCommitRecoveryError("unknown journal step")
...
class FilesystemJobStore:
    ...
    async def commit_success(
        ...
        async with self._lock:
            ...
            if not self._journal.ensure_step(journal, "COMPLETED"):
                raise FilesystemTaskCommitRecoveryError("commit did not complete")
    ...
    def _create_job_sync(self, job: JobRecord, root_task: TaskRecord) -> JobRecord:
        ...
        # for callers that bypass JobRuntime.
