#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""FilesystemRunStore: one JSON file per run under root/{run_id}.json. Atomic writes via
temp-file-then-os.replace, matching FileAssetBackend's write pattern.
...
def _to_json(record: RunRecord) -> dict:
    return {
        ...
        "input": {
            ...
            "metadata": dict(record.input.metadata),
        },
        "result": None
        if record.result is None
        else {
            "output": record.result.output,
            "token_usage": dict(record.result.token_usage),
        ...
        if record.error is None
            ...
            "detail": dict(record.error.detail),
        },
...
def _from_json(raw: dict) -> RunRecord:
    return RunRecord(
        ...
        runnable_type=RunnableType(raw["runnable_type"]),
        ...
        else datetime.fromisoformat(raw["started_at"]),
        finished_at=None
