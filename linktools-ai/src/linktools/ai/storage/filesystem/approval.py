#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""FilesystemApprovalStore: single-process file backend for ApprovalStore (the
Protocol in agent/approval.py). One JSON file per request at
root/requests/{approval_id}.json. Mirrors FilesystemMemoryStore/FilesystemRunStore's
atomic-write + path-traversal-guard patterns (see storage/filesystem/run.py). An
...
class FilesystemApprovalStore:
    ...
    def __init__(self, *, root: Path, metrics: "Any | None" = None) -> None:
        ...
        # of an existing (run_id, tool_call_id) approval with a different
