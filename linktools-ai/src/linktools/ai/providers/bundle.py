...
class ProviderPrefixes:
    ...
    packages: "str | None" = None  # packages need a filesystem root (see from_resources)
...
class ProviderBundle:
    ...
    tool_policies: "ToolPolicyProvider | None" = None
    ...
    def is_empty(self) -> bool:
        return not any(
            v is not None
            for v in (self.agents, self.skills, self.mcp_servers, self.tool_policies,
                      self.swarms, self.subagents, self.packages, self.package_resources,
                      self.entrypoints)
        )
