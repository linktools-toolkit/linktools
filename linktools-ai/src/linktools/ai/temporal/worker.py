...
from .activity.evaluation import EvaluationActivity
from .activity.execution import ExecuteActivity
from .activity.session import SessionActivity
from .activity.task import TaskActivity
from .workflow.evaluation import EvaluationWorkflow
from .workflow.execution import ExecutionWorkflow
from .workflow.session import SessionWorkflow
...
def build_temporal_components() -> 'tuple[TemporalSdkDataConverter, TemporalSdkPayloadCodec, TemporalSdkInterceptor]':
    if _TemporalDataConverter is None or _TemporalPayloadCodec is None or _TemporalInterceptor is None:
        raise LinktoolsAIError(ErrorCode.RUNTIME_DEPENDENCY_NOT_READY, "temporalio is required for production workers")
...
class WorkerRegistration:
    ...
    def register(self, worker: TemporalWorker) -> None:
        ...
        try:
            ...
            raise LinktoolsAIError(ErrorCode.RUNTIME_DEPENDENCY_NOT_READY) from error
        ...
        *,
        ...
        try:
            if data_converter.payload_codec is not payload_codec:
                raise LinktoolsAIError(ErrorCode.RUNTIME_DEPENDENCY_NOT_READY, "Temporal DataConverter must use the asset PayloadCodec")
            if client.config()["data_converter"] is not data_converter:
                raise LinktoolsAIError(ErrorCode.RUNTIME_DEPENDENCY_NOT_READY, "Temporal client must use the asset DataConverter")
