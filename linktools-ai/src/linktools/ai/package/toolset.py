...
def _check_allowed(
    package_id: str, allowed: "Mapping[str, PackageScope]"
) -> PackageScope:
        ...
        if emit is not None:
            await emit(
                PackageResourceListed(
                    package_id=package_id, path=path, count=len(result.items)
                )
            )
        ...
        if emit is not None:
            await emit(
                PackageEntrypointListed(
                    package_id=package_id, kind=kind, count=len(result.items)
                )
            )
        return result.model_dump()
