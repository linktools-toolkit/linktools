...
`CapabilityGroup.tool()` and `CapabilityGroup.capability()` accept a positive semantic `revision`. The revision is an explicit fingerprint input for Python behavior whose semantics cannot be reconstructed from a declaration payload. Generic Pydantic capabilities must also provide `semantic_config` (use `{}` when they have no configuration); their implementation identity and config are pinned into the Agent binding. Model selection/resolution and model-request interception, function/native tool contribution, tool-surface or tool-lifecycle interception, deferred loading or inline deferred-tool resolution, and `for_agent()` / `for_run()` replacement remain Runtime-owned boundaries. Other Pydantic behavior hooks remain available, and the final result is always revalidated against the durable `OutputBinding`. It is not a project-wide version layer.
