...
`CapabilityGroup.tool()` and `CapabilityGroup.capability()` accept a positive semantic `revision`. The revision is an explicit fingerprint input for Python behavior whose semantics cannot be reconstructed from a declaration payload. Generic Pydantic capabilities must also provide `semantic_config` (use `{}` when they have no configuration); their implementation identity and config are pinned into the Agent binding. Generic capabilities are static behavior extensions only: model selection/resolution and model-request interception, function/native tool contribution, tool-surface or tool-lifecycle interception, deferred loading, and `for_agent()` / `for_run()` replacement remain Runtime-owned boundaries. It is not a project-wide version layer.
