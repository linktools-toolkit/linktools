...
env/kernel wiring, FileSession/DbSession defaults, and coordinator sharing across
...
from linktools.ai.core.registry import AgentSpec, MCPServerSpec, SkillSpec, SpecSource, SubagentSpec
from linktools.ai.core.runtime import AgentKernel
from linktools.ai.core.session import (
    DbSession,
    DbSessionSpec,
    FileSession,
    FileSessionSpec,
...
from linktools.ai.core.stores import InMemorySessionStatusStore
from linktools.ai.core.session_coordination import InMemorySessionCoordinator, coordinator_for_store
from linktools.ai.core.session_window import NoopSummaryPolicy, RecentWindowPolicy
...
class _FakeAgentEnv:
    """Minimal `AgentEnvironment` test double: registries + `hooks`/`trace_root`/`workspace_root`."""
...
class _MinimalAgentRuntime:
    ...
    def session(self, trace_id: str, session_id: str = "main") -> Session:
        return Session.create(self.env, FileSessionSpec(trace_id=trace_id, session_id=session_id))
...
def test_agent_kernel_resolves_capability_bundle(tmp_path):
    ...
    context = AgentKernel(env).build_context(
        spec,
...
def test_agent_runtime_prepare_exposes_kernel_and_session_defaults(tmp_path: Path):
    ...
    assert runtime.kernel.environ is prepared_env
